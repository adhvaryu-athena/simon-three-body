...
def compute_n_sub_analytic(x, v, m, ii, jj, dt, G=4*np.pi**2, eta=1.0/30.0, max_substeps=64):
    ...
    Mirrors pair_eval_after_adaptive_analytic.py so the two integrators share one
    criterion. For the real Sun-Earth-Moon system the call passes G=cfg.G=4*pi^2.
    """
    rij = x[jj] - x[ii]
    vij = v[jj] - v[ii]
...
def simulate_simon_real_system(
    ...
    for _ in range(n_steps):
        ...
        if adaptive_enabled and r_min < adapt_thresh:
            # Dynamical-time criterion (analytic variant): n_sub from the
            # tightest pair's orbital/flyby timescale (see compute_n_sub_analytic).
            n_sub = compute_n_sub_analytic(x, v, m, ii, jj, float(dt), G=cfg.G, eta=eta, max_substeps=max_substeps)
...
def plot_metric_vs_dt(rows_on, rows_off, out_path, metric, ylabel, logy=True):
    ...
    ax.set_xlim(float(np.min(dts)) * 0.9, float(np.max(dts)) * 1.1)
    if logy:
        ax.set_yscale("log")
...
def write_csv(rows_on, rows_off, ias15_time, out_path):
    fieldnames = [
        ...
        "max_com", "energy_finite", "bounded", "lunar_orbit_preserved",
        "outcome", "finite",
    ]
...
def write_summary(
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            f"{'EM_max':>10} {'bounded':>8} {'lunar':>8} {'ejected':>8} "
            f"{'t_ej':>10}  {'outcome':<18}\n"
        )
        f.write("-" * 186 + "\n")
        for mode, rows in [("adaptive_ON", rows_on), ("adaptive_OFF", rows_off)]:
            for r in rows:
                speed = perf_ref["total_time_sec"] / max(r["runtime_sec"], 1e-30)
                t_ej = r["ejection_time"] if np.isfinite(r["ejection_time"]) else np.nan
                f.write(
                    ...
                    f"{str(r['bounded']):>8} {str(r['lunar_orbit_preserved']):>8} "
                    f"{str(r['ejected']):>8} {t_ej:>10.4f}  {r['outcome']:<18}\n"
                )
            f.write("\n")
        ...
        f.write("- bounded uses the paper's global criterion: all bodies remain within 10 AU of the COM.\n")
        f.write("- lunar indicates whether the Earth-Moon distance remains within a broad factor-of-two band around the ias15 Earth-Moon range.\n")
        f.write("- outcome is the reporting-safe classification: Valid, Bounded/degraded, Lunar orbit lost, or Ejected.\n")
        f.write("- A rollout can be globally bounded but still lose the lunar bound state; this is why both bounded and lunar are reported.\n")
...
def save_npz(
    ...
    np.savez_compressed(
        ...
        lunar_orbit_preserved_on=np.array([r["lunar_orbit_preserved"] for r in rows_on], dtype=bool),
        lunar_orbit_preserved_off=np.array([r["lunar_orbit_preserved"] for r in rows_off], dtype=bool),
        outcome_on=np.array([r["outcome"] for r in rows_on]),
        outcome_off=np.array([r["outcome"] for r in rows_off]),
...
def main():
    ...
    CASE_NAME = "sun_earth_moon_T100_selected_adaptive_frontier_energy_final"
    ...
    print(
        ...
        f"{'bounded':>7} | {'lunar':>7} | {'ejected':>7} | {'t_ej':>8} | {'outcome':<18}"
    )
    print("  " + "-" * 158)
    ...
    for dt in DT_VALUES:
        ...
        row_on = summarize_rollout(
            pos_on, vel_on, pos_ref, perf_on, "adaptive_ON", dt,
            m, cfg.G, cfg.eps, ref_em_min, ref_em_max
        )
        ...
        print(
            ...
            f"{str(row_on['bounded']):>7} | {str(row_on['lunar_orbit_preserved']):>7} | "
            f"{str(row_on['ejected']):>7} | "
        ...
        row_off = summarize_rollout(
            pos_off, vel_off, pos_ref, perf_off, "adaptive_OFF", dt,
            m, cfg.G, cfg.eps, ref_em_min, ref_em_max
        )
        ...
        print(
            ...
            f"{str(row_off['bounded']):>7} | {str(row_off['lunar_orbit_preserved']):>7} | "
            f"{str(row_off['ejected']):>7} | "
            f"{(row_off['ejection_time'] if np.isfinite(row_off['ejection_time']) else np.nan):>8.3f} | "
            f"{row_off['outcome']:<18}"
        )
