...
def compute_n_sub_analytic(x, v, m, ii, jj, dt, G=4*np.pi**2, eta=1.0/30.0, max_substeps=64):
    """Per-pair dynamical-time criterion. Tightest pair sets the rate.

    Defaults document the real-system configuration (G=4*pi^2 in AU/yr/Msun,
    eta=1/30, max_substeps=64) so the function is callable standalone in unit
    tests. The integrator's call site overrides all three explicitly.
    """
    rij = x[jj] - x[ii]                            # (n_pairs, 3)
    vij = v[jj] - v[ii]
    ...
    M_pair = m[ii] + m[jj]
    GM = G * M_pair
    t_orb   = 2.0 * np.pi * np.sqrt(r * r2 / (GM + 1e-30))
    t_flyby = r / vmag
    t_step  = np.minimum(t_orb, t_flyby)
    n_pair  = np.ceil(dt / (eta * t_step + 1e-30))
    n_max   = int(np.max(n_pair))
    return min(max_substeps, max(2, n_max))
...
def simulate_leapfrog_hybrid(x0, v0, m, model, cfg, dt, T, n_samples,
    ...
    max_substeps = 64     # max subdivisions per macro step
    eta = 1.0 / 30.0      # dynamical-time safety fraction (~1/eta steps per orbit)
    ...
    for _ in range(n_steps):
        ...
        if r_min < adapt_thresh:
            # Dynamical-time criterion: n_sub set by the tightest pair's
            # orbital/flyby timescale (see compute_n_sub_analytic).
            n_sub = compute_n_sub_analytic(x, v, m, ii, jj, dt_f, G=cfg.G, eta=eta, max_substeps=max_substeps)
            sub_dt = dt_f / n_sub
...
def plot_speed_accuracy_frontier(pts, op):
    ...
    with plt.rc_context({
        ...
        for p in pts:
            ...
            ax.annotate(
                f"dt={p['dt']}",
                xy=(p["throughput_samp_per_sec"], p["time_avg_rms"]),
                xytext=(dx, dy),
...
def main():
    ...
    for dt in dts:
        ...
        rms_t = rms_sep(pd, pr)
        fe = float(rms_t[-1])
        time_avg_rms = float(np.sqrt(np.mean(rms_t**2)))
        tp = ns/max(pf['total_time_sec'],1e-12)
        row={"dt":dt,"final_err":fe,"time_avg_rms":time_avg_rms,
             ...
             "total_substeps":pf["total_substeps"],"n_sub_history":pf["n_sub_history"]}
        
        frontier.append(row)
        s=perf_r['total_time_sec']/max(pf['total_time_sec'],1e-12)
        print(f"  dt={dt:>6} final_err={fe:.3e} time_avg_rms={time_avg_rms:.3e} "
              f"thrpt={tp:.0f} t/step={pf['time_per_step_sec']:.2e} "
    ...
    with open(os.path.join(args.out_dir,"perf_summary_T100.txt"),"w",encoding="utf-8") as f:
        ...
        _nsh = np.asarray(perf_m["n_sub_history"], dtype=np.int64)
        f.write(f"  total_substeps: {int(perf_m['total_substeps'])}\n")
        if _nsh.size:
            f.write(f"  n_sub_mean: {_nsh.mean():.4f}\n  n_sub_max: {int(_nsh.max())}\n")
        f.write(f"  n_sub_sum_equals_total: {int(_nsh.sum()) == int(perf_m['total_substeps'])}\n\n")
        f.write("Frontier sweep (hybrid):\ndt\tfinal_err\ttime_avg_rms\tthroughput(samp/s)\ttime_per_step(s)\ttotal_time(s)\tsteps\tfallback_frac\tspeedup\n")
        for p in frontier:
            s2=perf_r['total_time_sec']/max(p['total_time_sec'],1e-12)

            f.write(f"{p['dt']}\t{p['final_err']:.6e}\t{p['time_avg_rms']:.6e}\t"
                    f"{p['throughput_samp_per_sec']:.3f}\t"
        ...
        f.write("- time_avg_rms is sqrt(mean_t RMS(model - IAS15)^2) over 0–100 years.\n")
        ...
        f.write("\nSub-step statistics (analytic dynamical-time criterion):\n")
        f.write("dt\ttotal_substeps\tn_sub_mean\tn_sub_max\tn_sub_sum_equals_total\n")
        for p in frontier:
            h = np.asarray(p["n_sub_history"], dtype=np.int64)
            mean_ns = float(h.mean()) if h.size else 0.0
            max_ns = int(h.max()) if h.size else 0
            ok = int(h.sum()) == int(p["total_substeps"])
            f.write(f"{p['dt']}\t{p['total_substeps']}\t{mean_ns:.4f}\t{max_ns}\t{ok}\n")

    nsub_out = {
        "dt_rep": float(dr),
        "n_sub_history_rep": np.asarray(perf_m["n_sub_history"], dtype=np.int64),
        "total_substeps_rep": np.int64(perf_m["total_substeps"]),
        "frontier_dts": np.asarray([p["dt"] for p in frontier], dtype=np.float64),
        "frontier_total_substeps": np.asarray([p["total_substeps"] for p in frontier], dtype=np.int64),
    }
    for p in frontier:
        nsub_out[f"n_sub_history_dt_{p['dt']}".replace(".", "p")] = np.asarray(p["n_sub_history"], dtype=np.int64)
    np.savez(os.path.join(args.out_dir, "n_sub_history_T100.npz"), **nsub_out)
