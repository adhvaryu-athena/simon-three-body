"""
plot_simon_distance_thresholds_zone_methods_clear_v4.py

Clear two-panel version of the SIMON distance-threshold plot, updated for the
revised Zone 2 / Zone 3 methodology.

Key methodology update shown in the figure:
  1. Zone 2 is no longer a training-data / NN-correction region.
     It is handled by direct Newtonian force plus adaptive sub-stepping.
  2. Zone 3 is the trajectory-trained NN region.
     The NN learns a dt-aware scalar correction from one-step IAS15 comparison.
  3. Old analytic training ranges are retained only as historical context.

Run:
    python plot_simon_distance_thresholds_zone_methods_clear_v4.py

Outputs:
    simon_distance_thresholds_zone_methods_clear_v4.png
    simon_distance_thresholds_zone_methods_clear_v4.pdf
"""
...
# REVISED methodology ranges
ZONE2_START = R_GATE                  # 4e-4 AU
ZONE2_END   = ADAPT_THRESH            # 0.05 AU
Z3_TRAIN_START = ADAPT_THRESH + 0.002 # 0.052 AU
Z3_TRAIN_END   = NN_THRESH - 0.002    # 0.148 AU
...
def draw_range(ax, x_start, x_end, y, color, text, text_y_offset=0.18, lw=2.3):
    ...
    ax.text(
        ...
        fontsize=8.2,
        color=color,
        bbox=dict(boxstyle="round,pad=0.22", fc="white", ec="none", alpha=0.86),
        zorder=7,
    )
...
fig, (ax_top, ax_bottom) = plt.subplots(
    ...
    figsize=(18, 11),
    sharex=True,
    gridspec_kw={"height_ratios": [4.2, 1.8], "hspace": 0.08},
)

fig.suptitle(
    "SIMON distance thresholds and revised Zone 2 / Zone 3 methodology\n"
    "Zone 2 uses direct Newtonian + sub-stepping; Zone 3 uses trajectory-trained NN",
    y=0.965,
    fontsize=15,
)

# Leave enough bottom space so the lower panel is never cropped.
fig.subplots_adjust(left=0.15, right=0.965, top=0.900, bottom=0.125)
...
ax_top.set_ylim(0.25, len(systems) + 2.35)
...
callout(
    ...
    0.70,
    len(systems) + 0.38,
    "r_gate = 4×10⁻⁴ AU\nr_soft = r_soft_min\nbelow: fallback/direct Newtonian",
    "darkgreen",
)
callout(
    ax_top,
...
callout(
    ...
    0.70,
    len(systems) - 1.95,
    "nn_thresh = 0.15 AU\nZone 3 NN only between\n0.05 and 0.15 AU",
    "tab:blue",
)

# Compact softening reference box. Kept away from threshold labels.
ax_top.text(
    0.70,
    1.10,
    "Softening reference:\n"
    ...
    fontsize=8.3,
    color="#555555",
    bbox=dict(boxstyle="round,pad=0.30", fc="white", ec="#aaaaaa", alpha=0.90),
    zorder=9,
)
...
# =============================================================================
# Bottom panel: old training ranges and revised methodology ranges
# =============================================================================

add_zones(ax_bottom)
format_log_axis(ax_bottom)

ax_bottom.set_ylim(0.05, 5.25)
ax_bottom.set_yticks([4.35, 3.28, 2.08, 0.93])
ax_bottom.set_yticklabels(["OLD train 60%", "OLD train 40%", "Zone 2 method", "Zone 3 training"])

# Historical rows: retained only for context.
draw_range(
    ax_bottom,
    OLD_60_START,
    OLD_60_END,
    4.35,
    "forestgreen",
    "OLD: 0.5ε–50ε  (1.5e-4–1.5e-2 AU)\nanalytic softening-correction training",
    text_y_offset=0.15,
    lw=2.0,
)
...
draw_range(
    ...
    3.28,
    "#003cb3",
    "OLD: 50ε–10 AU  (1.5e-2–10 AU)\nfar-field analytic samples",
    text_y_offset=0.15,
    lw=2.0,
)

# Revised Zone 2: no trajectory training; use exact force with sub-stepping. Shown in red to match revised-method emphasis.
draw_range(
    ax_bottom,
    ZONE2_START,
    ZONE2_END,
    2.08,
    "#b00020",
    "REVISED ZONE 2: 4e-4–0.05 AU\nNO NN training here\nforce = direct Newtonian; stability from sub-steps",
    text_y_offset=0.13,
    lw=2.7,
)

# Revised Zone 3: trajectory-trained NN.
draw_range(
    ax_bottom,
    Z3_TRAIN_START,
    Z3_TRAIN_END,
    0.93,
    "#8b0000",
    "REVISED ZONE 3: 0.052–0.148 AU\ntrajectory-trained NN\ninput includes log(dt); target = c_opt vs IAS15",
    text_y_offset=0.13,
    lw=2.7,
)
...
ax_bottom.text(
    22,
    4.95,
    "Reading guide:\nTop = thresholds and example systems\nBottom = old training context + revised methods\nZone 2 is now a method region, not a training-data region",
    ha="left",
    va="top",
    fontsize=8.3,
    color="#333333",
...
out_png = "simon_distance_thresholds_zone_methods_clear_v4.png"
out_pdf = "simon_distance_thresholds_zone_methods_clear_v4.pdf"
