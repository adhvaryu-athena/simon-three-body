...
# =============================================================================
# Revised 3-way comparison helpers and Main
# =============================================================================
def parse_previous_summary(path: str) -> Tuple[Optional[float], List[Dict[str, float]]]:
    """Parse old Run D perf_summary_T100_timeavg.txt robustly.

    Supports the original table format:
        dt final_err time_avg_rms throughput time_per_step total_time steps fallback_frac speedup
    where speedup may be written as "1.02x".
    """
    if not path or not os.path.exists(path):
        return None, []
    ias_time = None
            ...
            if in_table:
                ...
                if len(parts) >= 8:
                    try:
                        speedup = np.nan
                        if len(parts) >= 9:
                            speedup = float(parts[8].replace("x", ""))
                        pts.append({
                            "dt": float(parts[0]),
                            "final_err": float(parts[1]),
                            "time_avg_rms": float(parts[2]),
...
def make_threeway_rows(old_pts, no_nn_pts, v4_pts):
    old_by_dt = {round(float(p["dt"]), 6): p for p in old_pts}
    no_by_dt = {round(float(p["dt"]), 6): p for p in no_nn_pts}
    v4_by_dt = {round(float(p["dt"]), 6): p for p in v4_pts}
    all_dts = sorted(set(old_by_dt) | set(no_by_dt) | set(v4_by_dt))
    rows = []
    for d in all_dts:
        old = old_by_dt.get(d, {})
        no = no_by_dt.get(d, {})
        ...
        row = {
            "dt": d,
            "old_time_avg_rms": old.get("time_avg_rms", np.nan),
            "no_nn_time_avg_rms": no.get("time_avg_rms", np.nan),
            "v4_time_avg_rms": v4.get("time_avg_rms", np.nan),
            "old_final_err": old.get("final_err", np.nan),
            "no_nn_final_err": no.get("final_err", np.nan),
            "v4_final_err": v4.get("final_err", np.nan),
            "old_speedup": old.get("speedup", np.nan),
            "no_nn_speedup": no.get("speedup", np.nan),
            "v4_speedup": v4.get("speedup", np.nan),
        }
        # Positive means v4 worse than no-NN; negative means v4 improves over no-NN.
        if np.isfinite(row["no_nn_time_avg_rms"]) and np.isfinite(row["v4_time_avg_rms"]):
            row["v4_vs_no_nn_time_avg_change_pct"] = 100.0 * (row["v4_time_avg_rms"] - row["no_nn_time_avg_rms"]) / max(abs(row["no_nn_time_avg_rms"]), 1e-30)
        else:
            row["v4_vs_no_nn_time_avg_change_pct"] = np.nan
        if np.isfinite(row["old_time_avg_rms"]) and np.isfinite(row["v4_time_avg_rms"]):
            row["v4_vs_old_time_avg_change_pct"] = 100.0 * (row["v4_time_avg_rms"] - row["old_time_avg_rms"]) / max(abs(row["old_time_avg_rms"]), 1e-30)
        else:
            row["v4_vs_old_time_avg_change_pct"] = np.nan
        rows.append(row)
    return rows
...
def _fmt_speed(x):
    return "nan" if not np.isfinite(x) else f"{x:.2f}x"


def plot_speed_accuracy_frontier_3way(v4_pts, no_nn_pts, op, old_pts: Optional[List[Dict[str, float]]] = None):
    with plt.rc_context({"font.size": 10, "axes.labelsize": 10, "xtick.labelsize": 8, "ytick.labelsize": 8, "legend.fontsize": 8}):
        ...
        if old_pts:
            ...
            ax.plot(ox, oy, linestyle="--", lw=1.0, alpha=0.6)
        if no_nn_pts:
            nx = [p["throughput_samp_per_sec"] for p in no_nn_pts]
            ny = [p["time_avg_rms"] for p in no_nn_pts]
            ax.scatter(nx, ny, marker="s", s=36, zorder=3, label="revised no Zone 3 NN")
            ax.plot(nx, ny, lw=1.0, alpha=0.6)
        vx = [p["throughput_samp_per_sec"] for p in v4_pts]
        vy = [p["time_avg_rms"] for p in v4_pts]
        ax.scatter(vx, vy, marker="o", s=42, zorder=3, label="v4 Zone 3 frozen-c NN")
        ax.plot(vx, vy, lw=1.0, alpha=0.6)
        for p in v4_pts:
            ax.annotate(f"dt={p['dt']}", xy=(p["throughput_samp_per_sec"], p["time_avg_rms"]),
        ...
        ax.set_yscale("log")
        ax.set_xlabel("Throughput (recorded samples / sec, log)")
        ax.set_ylabel("Time-averaged RMS deviation vs IAS15 over 0–T (log)")
        ax.grid(True, which="both", alpha=0.25)
        ax.legend()
        plt.tight_layout()
        plt.savefig(op, dpi=300)
...
def run_frontier(label, x0, v0, m, model, cfg, dts, T, ns, pr, ias15_time, use_zone3_nn: bool):
    rows = []
    for dt_val in dts:
        _, pd, _, pf = simulate_leapfrog_v4_zone3(
            x0, v0, m, model, cfg, dt_val, T, ns,
            use_zone3_nn=use_zone3_nn,
            label=label,
        )
        rms_t = rms_sep(pd, pr)
        fe = float(rms_t[-1])
        ...
        s2 = ias15_time / max(pf["total_time_sec"], 1e-12)
        row = {
            "dt": dt_val,
            "final_err": fe,
            "time_avg_rms": tar,
            ...
            "avg_fallback_frac": pf["zone3_nn_frac"] if use_zone3_nn else pf["zone3_no_nn_frac"],
            "zone2_frac": pf["zone2_frac"],
            "zone3_frac": pf["zone3_frac"],
            "zone3_nn_frac": pf["zone3_nn_frac"],
            "zone3_no_nn_frac": pf["zone3_no_nn_frac"],
        ...
        rows.append(row)
        ztxt = f"zone3NN={pf['zone3_nn_frac']:.4f}" if use_zone3_nn else f"zone3NoNN={pf['zone3_no_nn_frac']:.4f}"
        print(f"  [{label}] dt={dt_val:>6} final_err={fe:.3e} time_avg_rms={tar:.3e} "
              f"thrpt={tp:.0f} total={pf['total_time_sec']:.3f}s "
              f"zone2={pf['zone2_frac']:.4f} {ztxt} speedup={s2:.2f}x")
    return rows
...
def main():
    ...
    parser.add_argument("--out_dir", default="v4_zone3_frozen_eval_3way_out")
    ...
    parser.add_argument("--skip_no_nn_frontier", action="store_true",
                        help="Skip the full revised no-Zone-3-NN frontier. Not recommended for Phase 2 analysis.")
    ...
    if not model_path.endswith(".pt") and os.path.exists(model_path + ".pt"):
        model_path += ".pt"
    model = load_v4_model(model_path)
    ...
    print("[eval-v4-3way] IAS15 baseline...")
    tr, pr, vr_ref, perf_r = simulate_rebound_ias15(x0, v0, m, cfg.G, T, ns)
    ias15_time = perf_r["total_time_sec"]
    print(f"[eval-v4-3way] IAS15 time={ias15_time:.3f}s")

    dr = float(args.dt_rep)
    print(f"[eval-v4-3way] Representative v4 Zone 3 run dt={dr}...")
    tm, pm, vm, perf_m = simulate_leapfrog_v4_zone3(x0, v0, m, model, cfg, dr, T, ns, use_zone3_nn=True, label="v4_zone3_nn")
    sp = ias15_time / max(perf_m["total_time_sec"], 1e-12)
    delta = rms_sep(pm, pr)
    slope, win = fit_log_slope(tr, delta)
    final_err = float(delta[-1])
    time_avg_rms = float(np.sqrt(np.mean(delta**2)))
    print(f"[eval-v4-3way] v4 dt={dr}: final_err={final_err:.3e} time_avg_rms={time_avg_rms:.3e} "
    ...
    print(f"[eval-v4-3way] Representative revised no-Zone-3-NN baseline dt={dr}...")
    _, p_no_rep, _, perf_no_rep = simulate_leapfrog_v4_zone3(x0, v0, m, model, cfg, dr, T, ns, use_zone3_nn=False, label="zone3_no_nn")
    delta_no_rep = rms_sep(p_no_rep, pr)
    no_nn_rep = {
        "perf": perf_no_rep,
        "final_err": float(delta_no_rep[-1]),
        "time_avg_rms": float(np.sqrt(np.mean(delta_no_rep**2))),
        "speedup": ias15_time / max(perf_no_rep["total_time_sec"], 1e-12),
    }
    print(f"[eval-v4-3way] no-Zone-3-NN dt={dr}: final_err={no_nn_rep['final_err']:.3e} "
          f"time_avg_rms={no_nn_rep['time_avg_rms']:.3e} speedup={no_nn_rep['speedup']:.2f}x")
    plot_two_divergence(tr, delta, delta_no_rep, os.path.join(args.out_dir, "v4_vs_zone3_no_nn_divergence_T100.png"))

    old_summary_path = resolve_old_summary_path(args.old_summary)
    old_ias_time, old_pts = parse_previous_summary(old_summary_path)

    print("[eval-v4-3way] Frontier sweep: v4 Zone 3 frozen-c NN...")
    v4_frontier = run_frontier("v4", x0, v0, m, model, cfg, dts, T, ns, pr, ias15_time, use_zone3_nn=True)

    no_nn_frontier = []
    if not args.skip_no_nn_frontier:
        print("[eval-v4-3way] Frontier sweep: revised no-Zone-3-NN baseline...")
        no_nn_frontier = run_frontier("no_zone3_nn", x0, v0, m, model, cfg, dts, T, ns, pr, ias15_time, use_zone3_nn=False)
    else:
        print("[eval-v4-3way] Skipped no-Zone-3-NN frontier by user request.")

    plot_speed_accuracy_frontier_3way(
        v4_frontier,
        no_nn_frontier,
        os.path.join(args.out_dir, "speed_accuracy_frontier_T100_3way.png"),
        old_pts=old_pts,
    )
    plot_comp_cost_frontier(v4_frontier, os.path.join(args.out_dir, "comp_cost_frontier_T100_v4.png"))

    comparison_rows = make_comparison_rows(old_pts, v4_frontier)
    threeway_rows = make_threeway_rows(old_pts, no_nn_frontier, v4_frontier)
    plot_old_vs_new_bars(comparison_rows, os.path.join(args.out_dir, "old_runD_vs_v4_timeavg_rms.png"))

    # Main summary.
    summary_path = os.path.join(args.out_dir, "perf_summary_T100_v4_zone3_3way.txt")
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(f"T_years: {T}\n")
        f.write(f"n_samples: {ns}\n")
        f.write("method: Revised Zone 2/Zone 3 frozen-c, with full three-way frontier comparison\n")
        ...
        f.write("zone3 v4: 0.05 AU <= r < 0.15 AU -> bounded v4 trajectory NN frozen per macro/sub-step\n")
        f.write("zone3 no-NN baseline: 0.05 AU <= r < 0.15 AU -> c=1 times F_soft; Zone 2 remains direct Newtonian\n")
        f.write("zone4: r >= 0.15 AU -> direct Newtonian\n\n")
        f.write(f"Model: {model_path}\n\n")
        f.write("Baseline (REBOUND IAS15):\n")
        f.write(f"  total_time_sec: {ias15_time:.6f}\n\n")
        ...
        f.write(f"Representative revised no-Zone-3-NN baseline (dt_rep={dr}):\n")
        f.write(f"  final_err: {no_nn_rep['final_err']:.6e}\n")
        f.write(f"  time_avg_rms: {no_nn_rep['time_avg_rms']:.6e}\n")
        f.write(f"  speedup_vs_ias15: {no_nn_rep['speedup']:.2f}x\n")
        imp = 100.0 * (time_avg_rms - no_nn_rep["time_avg_rms"]) / max(abs(no_nn_rep["time_avg_rms"]), 1e-30)
        f.write(f"  v4_vs_no_nn_time_avg_change_pct: {imp:+.2f}%\n\n")
        ...
        if no_nn_frontier:
            f.write("\nFrontier sweep (revised no-Zone-3-NN baseline):\n")
            f.write("dt\tfinal_err\ttime_avg_rms\tthroughput(samp/s)\ttime_per_step(s)\ttotal_time(s)\tsteps\tzone2_frac\tzone3_no_nn_frac\tspeedup\n")
            for p in no_nn_frontier:
                f.write(f"{p['dt']}\t{p['final_err']:.6e}\t{p['time_avg_rms']:.6e}\t"
                        f"{p['throughput_samp_per_sec']:.3f}\t{p['time_per_step_sec']:.6e}\t"
                        f"{p['total_time_sec']:.6f}\t{p['steps']}\t{p['zone2_frac']:.6f}\t"
                        f"{p['zone3_no_nn_frac']:.6f}\t{p['speedup']:.2f}x\n")

        f.write("\nThree-way frontier comparison:\n")
        f.write("dt\told_time_avg\tnoNN_time_avg\tv4_time_avg\tv4_vs_noNN_%\tv4_vs_old_%\told_final\tnoNN_final\tv4_final\told_speedup\tnoNN_speedup\tv4_speedup\n")
        for r in threeway_rows:
            f.write(f"{r['dt']}\t{_fmt_float(r['old_time_avg_rms'])}\t{_fmt_float(r['no_nn_time_avg_rms'])}\t{_fmt_float(r['v4_time_avg_rms'])}\t"
                    f"{_fmt_pct(r['v4_vs_no_nn_time_avg_change_pct'])}\t{_fmt_pct(r['v4_vs_old_time_avg_change_pct'])}\t"
                    f"{_fmt_float(r['old_final_err'])}\t{_fmt_float(r['no_nn_final_err'])}\t{_fmt_float(r['v4_final_err'])}\t"
                    f"{_fmt_speed(r['old_speedup'])}\t{_fmt_speed(r['no_nn_speedup'])}\t{_fmt_speed(r['v4_speedup'])}\n")

        f.write("\nNotes:\n")
        f.write("- final_err is RMS(model - IAS15) at T years.\n")
        f.write("- time_avg_rms is sqrt(mean_t RMS(model - IAS15)^2) over 0-T.\n")
        f.write("- Negative v4_vs_noNN_% means v4 improves time_avg_rms over the revised no-Zone-3-NN baseline.\n")
        f.write("- Negative v4_vs_old_% means v4 improves time_avg_rms over previous Run D.\n")
        f.write("- The no-Zone-3-NN baseline uses c=1 times F_soft in Zone 3, while Zone 2 remains direct Newtonian + sub-stepping.\n")

    # Old-vs-v4 comparison summary.
        ...
        if not comparison_rows:
            ...
            for r in comparison_rows:
                f.write(f"{r['dt']}\t{r['old_time_avg_rms']:.6e}\t{r['new_time_avg_rms']:.6e}\t"
                        ...
                        f"{_fmt_speed(r.get('old_speedup', np.nan))}\t{_fmt_speed(r.get('new_speedup', np.nan))}\n")
            f.write("\nNegative change means v4 improved the metric. Positive change means worse.\n")

    # Three-way comparison summary.
    three_path = os.path.join(args.out_dir, "comparison_old_vs_noNN_vs_v4.txt")
    with open(three_path, "w", encoding="utf-8") as f:
        f.write("Three-way comparison: previous Run D vs revised no-Zone-3-NN vs v4 Zone 3 frozen-c\n")
        f.write(f"Previous summary file: {old_summary_path if os.path.exists(old_summary_path) else 'NOT FOUND'}\n")
        f.write("No-Zone-3-NN baseline: Zone 2 direct Newtonian + sub-stepping; Zone 3 c=1 times F_soft.\n\n")
        f.write("dt\told_time_avg\tnoNN_time_avg\tv4_time_avg\tv4_vs_noNN_%\tv4_vs_old_%\told_final\tnoNN_final\tv4_final\told_speedup\tnoNN_speedup\tv4_speedup\n")
        for r in threeway_rows:
            f.write(f"{r['dt']}\t{_fmt_float(r['old_time_avg_rms'])}\t{_fmt_float(r['no_nn_time_avg_rms'])}\t{_fmt_float(r['v4_time_avg_rms'])}\t"
                    f"{_fmt_pct(r['v4_vs_no_nn_time_avg_change_pct'])}\t{_fmt_pct(r['v4_vs_old_time_avg_change_pct'])}\t"
                    f"{_fmt_float(r['old_final_err'])}\t{_fmt_float(r['no_nn_final_err'])}\t{_fmt_float(r['v4_final_err'])}\t"
                    f"{_fmt_speed(r['old_speedup'])}\t{_fmt_speed(r['no_nn_speedup'])}\t{_fmt_speed(r['v4_speedup'])}\n")
        f.write("\nInterpretation: negative percentage means v4 improves time_avg_rms; positive means v4 is worse.\n")

    print(f"[eval-v4-3way] wrote {args.out_dir}")
    print(f"[eval-v4-3way] main summary: {summary_path}")
    print(f"[eval-v4-3way] old-vs-v4 comparison: {comp_path}")
    print(f"[eval-v4-3way] three-way comparison: {three_path}")
    print("[eval-v4-3way] done.")
