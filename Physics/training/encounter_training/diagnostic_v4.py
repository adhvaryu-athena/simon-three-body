...
Comprehensive diagnostics for the SIMON Zone 3 v4 bounded 6-input correction model.
...
Example:
  python -B diagnostic_v4.py --data encounter_data_zone3_v3.npz --model pair_correction_nn_v4_bounded.pt
"""
...
class PairCorrectionNN(nn.Module):
    """
    v4 bounded 6-input correction model.

    The network predicts a raw scalar and maps it through a sigmoid so that
    c_pred is always inside [c_min, c_max]. The forward method returns log(c),
    which keeps the rest of the diagnostic logic unchanged while enforcing
    the v4 output bounds.
    """
    def __init__(self, hidden: int = 64, c_min: float = 0.25, c_max: float = 3.0):
        super().__init__()
        if not (0.0 < float(c_min) < float(c_max)):
            raise ValueError(f"Invalid v4 bounds: c_min={c_min}, c_max={c_max}")
...
def load_model(model_path: str) -> PairCorrectionNN:
    ...
    if "log_c_min" not in sd or "log_c_max" not in sd:
        raise KeyError(
            "This state_dict does not contain log_c_min/log_c_max. "
            "It is not a v4 bounded model; use diagnostic_v4.py for v3 models."
        )

    c_min = float(torch.exp(sd["log_c_min"]).cpu().item())
    c_max = float(torch.exp(sd["log_c_max"]).cpu().item())
    model = PairCorrectionNN(hidden=hidden, c_min=c_min, c_max=c_max)
    model.load_state_dict(sd)
    model.eval()

    if model.input_mean.numel() != 6 or model.input_std.numel() != 6:
        raise ValueError("Model normalisation buffers are not length 6; this is not a v4 6-input model.")
    if not (torch.isfinite(model.log_c_min) and torch.isfinite(model.log_c_max)):
        raise ValueError("Model bounds are non-finite.")
    if not float(model.log_c_min) < float(model.log_c_max):
        raise ValueError("Model bounds are invalid: log_c_min >= log_c_max.")

    print(f"[model] Loaded {model_path}")
    print("[model] type=v4 bounded output")
    print(f"[model] hidden={hidden}, parameters={sum(p.numel() for p in model.parameters())}")
    print(f"[model] c bounds=[{c_min:.6g}, {c_max:.6g}]")
...
def maybe_make_plots(raw: Dict[str, np.ndarray], idx: np.ndarray, y_log: np.ndarray, pred_log: np.ndarray, out_dir: str) -> None:
    ...
    ax.set_title("Zone 3 v4 held-out predicted vs true c")
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "diag_v4_pred_vs_true.png"), dpi=180)
    plt.close(fig)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "diag_v4_median_by_dt.png"), dpi=180)
    plt.close(fig)
    ...
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "diag_v4_residual_vs_vrad.png"), dpi=180)
    plt.close(fig)
    ...
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "diag_v4_velocity_space_pred.png"), dpi=180)
    plt.close(fig)
...
def final_verdict(
    ...
    return "PROCEED TO ROLLOUT TESTS: held-out and probe diagnostics support the v4 bounded model."


def main() -> None:
    ap = argparse.ArgumentParser(description="Diagnose SIMON Zone 3 v4 bounded 6-input correction model.")
    ap.add_argument("--data", default="encounter_data_zone3_v3_augmented.npz")
    ap.add_argument("--model", default="pair_correction_nn_v4_bounded.pt")
    ap.add_argument("--split", default="pair_correction_nn_v4_bounded_split.npz")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--out-dir", default="diagnostic_v4_out")
