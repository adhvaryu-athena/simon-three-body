...
def simulate_leapfrog_v4_zone3(
    ...
    """
    Revised evaluator with FROZEN macro-step Zone-3 correction.

    This version matches the Zone-3 training target more closely than the first
    v4 evaluator.  The training generator chooses one c_opt for one full
    leapfrog macro-step.  Therefore, at the beginning of each macro/sub-step we:

      1. inspect the current state (x_n, v_n),
      2. compute one c value for each pair currently in Zone 3,
      3. freeze those c values for both acceleration calls in that step.

    Zone logic:
      Zone 1: r < 4e-4 AU       -> direct Newtonian hard fallback
      Zone 2: 4e-4 <= r < 0.05  -> direct Newtonian + adaptive sub-stepping, no NN
      Zone 3: 0.05 <= r < 0.15  -> bounded v4 trajectory NN frozen per macro/sub-step, frozen per step
    ...
    stats = {
        ...
        "c_min": np.inf,
        "c_max": -np.inf,
        "frozen_steps_with_zone3": 0,
    }

    def _zone3_features(pos, vel, step_dt, zmask):
        """Build v4 features for pairs selected by zmask at the START of a step."""
        rij = pos[jj] - pos[ii]
        vij = vel[jj] - vel[ii]
        ...
        r_hat = rij[zmask] / (r[zmask, None] + 1e-30)
        v_rad = np.einsum("ij,ij->i", vij[zmask], r_hat)
        v_tan_vec = vij[zmask] - v_rad[:, None] * r_hat
        v_tan = np.sqrt(np.einsum("ij,ij->i", v_tan_vec, v_tan_vec) + 1e-30)
        v_scale = np.sqrt(G * (mi_arr[zmask] + mj_arr[zmask]) / (r[zmask] + 1e-30))
        v_rad_norm = v_rad / (v_scale + 1e-30)
        v_tan_norm = v_tan / (v_scale + 1e-30)

        nn_in = np.empty((int(np.sum(zmask)), 6), dtype=np.float32)
    ...
    def make_frozen_zone3_state(pos, vel, step_dt):
        """
        Compute one frozen c per pair for the coming step.
        Returned c values are only meaningful where eligible=True.
        """
        ...
        if use_zone3_nn:
            nn_in, v_rad_norm = _zone3_features(pos, vel, step_dt, z0)
            ...
            stats["frozen_steps_with_zone3"] += 1
    ...
    def compute_acc(pos, frozen_c, eligible, fallback, strong):
        rij = pos[jj] - pos[ii]
        r2 = np.einsum("ij,ij->i", rij, rij)
        r = np.sqrt(r2 + 1e-30)
        ...
        # Apply frozen Zone-3 c only for pairs that were eligible at the start
        # of this macro/sub-step and are still in Zone 3 at this force call.
        ...
        if np.any(apply_mask):
            ...
            if use_zone3_nn:
                ...
                stats["zone3_nn_pairs"] += int(np.sum(apply_mask))
                stats["zone3_gate_fallback_pairs"] += int(np.sum(fb))
                stats["strong_approach_zone3_pairs"] += int(np.sum(strong[apply_mask]))
            else:
                F_scalar[apply_mask] = F_soft
                stats["zone3_no_nn_pairs"] += int(np.sum(apply_mask))
    ...
    def verlet_frozen_step(x_in, v_in, step_dt):
        frozen_c, eligible, fallback, strong = make_frozen_zone3_state(x_in, v_in, step_dt)
        a0 = compute_acc(x_in, frozen_c, eligible, fallback, strong)
        vh = v_in + 0.5 * step_dt * a0
        x_new = x_in + step_dt * vh
        a1 = compute_acc(x_new, frozen_c, eligible, fallback, strong)
        v_new = vh + 0.5 * step_dt * a1
        return x_new, v_new

    def compute_acc_far(pos):
        """Simplified force computation for guaranteed Zone 4 steps (r_min >= nn_thresh).

        When r_min >= nn_thresh (0.15 AU) every pair has r >= 0.15 AU, so Zone 1,
        Zone 2, and Zone 3 are all empty. Only direct Newtonian is needed.

        Removes four zone-mask computations, three unnecessary stats int() conversions,
        and the apply_mask / np.any block that run on every compute_acc call even when
        no zone-specific branches are taken. Stats are updated only for pair_evals and
        far_pairs (both guaranteed correct for this regime).
        Physics is identical to compute_acc with eligible=all-False."""
        rij      = pos[jj] - pos[ii]
        r2       = np.einsum("ij,ij->i", rij, rij)
        r        = np.sqrt(r2 + 1e-30)
        F_scalar = Gmimj / (r2 * r + 1e-30)   # exact Newtonian, no softening

        stats["pair_evals"] += P   # all P pairs evaluated
        stats["far_pairs"]  += P   # all P pairs are guaranteed far
    ...
    def verlet_no_zone3(x_in, v_in, step_dt):
        """Fast verlet step for r_min >= nn_thresh.
        Uses compute_acc_far which skips zone masking and all NN logic.
        Physics identical to compute_acc with no Zone-3 pairs present."""
        a0    = compute_acc_far(x_in)
        vh    = v_in + 0.5 * step_dt * a0
        x_new = x_in + step_dt * vh
        a1    = compute_acc_far(x_new)
        v_new = vh + 0.5 * step_dt * a1
        return x_new, v_new
    ...
    for _ in range(n_steps):
        ...
        if r_min < cfg.adapt_thresh:
            # Zone 2 active: sub-step.  Zone 3 NN may also activate during
            # sub-steps (pairs can cross from r<0.05 to r in [0.05,0.15]),
            # so use verlet_frozen_step for every sub-step.
            n_sub = min(cfg.max_substeps, max(2, int(np.ceil(cfg.adapt_thresh / r_min))))
            sub_dt = dt_f / n_sub
            for _ in range(n_sub):
                x, v = verlet_frozen_step(x, v, sub_dt)
            total_substeps += n_sub
        elif r_min < cfg.nn_thresh:
            # Zone 3 range (0.05 <= r_min < 0.15): at least one pair may be
            # in Zone 3.  Compute frozen-c via make_frozen_zone3_state.
            x, v = verlet_frozen_step(x, v, dt_f)
            total_substeps += 1
        else:
            # Zone 4 (r_min >= 0.15): all pairs use direct Newtonian.
            # Skip make_frozen_zone3_state entirely -- this is ~99.9% of steps.
            x, v = verlet_no_zone3(x, v, dt_f)
            total_substeps += 1
    ...
    perf = {
        ...
        "frozen_steps_with_zone3": stats["frozen_steps_with_zone3"],
