"""
train_pair_correction_v5_blended.py

Train the bounded, velocity-aware Zone 3 scalar correction NN for SIMON using optional blended sample weights.
...
def load_zone3_v3_data(npz_path: str, c_min: float, c_max: float) -> DatasetBundle:
    ...
    # Optional metadata added by make_zone3_v5_blended_dataset.py.
    # The normal evaluator ignores these fields, but the blended trainer uses
    # sample_weight so window-target samples can have higher influence than
    # old one-step samples without physically duplicating rows.
    for opt_key in ["sample_weight", "source_id", "target_type_id"]:
        if opt_key in data.files:
            raw[opt_key] = data[opt_key].astype(np.float32)
    n = len(raw["r_AU"])
    ...
    if "sample_weight" in raw:
        sw = raw["sample_weight"].astype(np.float64)
        print("[data] optional blended metadata detected")
        print(f"       sample_weight min/med/max={sw.min():.3f}/{np.median(sw):.3f}/{sw.max():.3f}  mean={sw.mean():.3f}")
        if "source_id" in raw:
            sid = raw["source_id"].astype(np.int64)
            for src in sorted(set(sid.tolist())):
                mask = sid == src
                label = "old_one_step" if src == 0 else "window_target" if src == 1 else f"source_{src}"
                print(f"       source {src} ({label}): n={int(mask.sum())} effective_weight={float(sw[mask].sum()):.1f}")
...
def make_splits_stratified(raw: Dict[str, np.ndarray], seed: int, train_frac: float = 0.70, val_frac: float = 0.15) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Stratified split for blended datasets.

    If source_id is present, split separately by (source_id, rounded dt) so the
    train/val/test sets all contain both old one-step and window-target samples.
    This avoids a random split accidentally under-representing the much smaller
    window subset. If source_id is absent, fall back to the original random split.
    """
    n = len(raw["r_AU"])
    if "source_id" not in raw:
        return make_splits(n, seed=seed, train_frac=train_frac, val_frac=val_frac)

    rng = np.random.RandomState(seed)
    sid = raw["source_id"].astype(np.int64)
    dt = _round_dt(raw["log_dt"].astype(np.float64))
    train_parts, val_parts, test_parts = [], [], []

    for key in sorted(set(zip(sid.tolist(), dt.tolist()))):
        src, d = key
        idx = np.where((sid == src) & (dt == d))[0]
        idx = rng.permutation(idx)
        n_g = len(idx)
        if n_g < 3:
            train_parts.append(idx)
            continue
        n_train = int(round(train_frac * n_g))
        n_val = int(round(val_frac * n_g))
        # Ensure at least one test example where possible.
    ...
    if len(test_idx) == 0:
        raise ValueError("Test split is empty; use a larger dataset")
    print("[split] Stratified by source_id and dt")
...
def make_sample_weights(raw: Dict[str, np.ndarray], indices: np.ndarray,
    ...
    if "sample_weight" in raw:
        dataset_w = raw["sample_weight"][indices].astype(np.float32)
        if not np.all(np.isfinite(dataset_w)) or np.any(dataset_w <= 0):
...
def train_model(bundle: DatasetBundle, hidden: int, epochs: int, lr: float, seed: int,
    ...
    n = len(features)
    train_idx, val_idx, test_idx = make_splits_stratified(bundle.raw, seed=seed)
    ...
    if "source_id" in bundle.raw:
        sid_train = bundle.raw["source_id"][train_idx].astype(np.int64)
        for src in sorted(set(sid_train.tolist())):
            mask = sid_train == src
            label = "old_one_step" if src == 0 else "window_target" if src == 1 else f"source_{src}"
            print(f"  source {src} ({label}): n={int(mask.sum())} effective_weight={float(train_weights[mask].sum()):.1f}")
...
def write_summary(path: str, args: argparse.Namespace, metrics: Dict[str, float]) -> None:
    with open(path, "w", encoding="utf-8") as f:
        f.write("Zone 3 v5 blended/window bounded training summary\n")
        f.write("=" * 72 + "\n")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train SIMON Zone 3 v5 blended/window bounded 6-input trajectory-correction NN.")
    ...
    ap.add_argument("--output", default="pair_correction_nn_v5_blended_weighted.pt")
    ...
    summary_path = "train_pair_correction_v5_blended_summary.txt"
