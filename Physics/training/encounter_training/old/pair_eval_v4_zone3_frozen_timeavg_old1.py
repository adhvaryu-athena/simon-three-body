...
def simulate_leapfrog_v4_zone3(
    ...
    # Pre-allocated trivial frozen state for Zone 4 steps (r_min >= nn_thresh).
    # When all pairs are beyond nn_thresh, zone3_mask is guaranteed all-False,
    # so eligible=False causes apply_mask=False and compute_acc uses direct
    # Newtonian for every pair -- identical physics, zero NN overhead.
    _fc_ones  = np.ones(P,  dtype=np.float64)
    _el_false = np.zeros(P, dtype=bool)
    _fb_false = np.zeros(P, dtype=bool)
    _st_false = np.zeros(P, dtype=bool)

    def verlet_no_zone3(x_in, v_in, step_dt):
        """Fast verlet step for r_min >= nn_thresh.
        All pairs are in Zone 4 (direct Newtonian). Skips make_frozen_zone3_state
        entirely, eliminating the per-step overhead on ~99.9% of integration steps.
        The pre-allocated eligible=False arrays ensure compute_acc never applies
        Zone-3 corrections, so the physics is identical to the full path with an
        empty Zone-3 set."""
        a0 = compute_acc(x_in, _fc_ones, _el_false, _fb_false, _st_false)
        vh = v_in + 0.5 * step_dt * a0
        x_new = x_in + step_dt * vh
        a1 = compute_acc(x_new, _fc_ones, _el_false, _fb_false, _st_false)
        v_new = vh + 0.5 * step_dt * a1
        return x_new, v_new
