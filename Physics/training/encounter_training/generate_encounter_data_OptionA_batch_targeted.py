...
Sampling modes
--------------
    broad            : original v3 sampling, with random pair velocity direction.
    strong_approach  : targeted supplemental sampling for the weak region found
                       by diagnostics: large-dt, strongly approaching Zone-3
                       states with moderate tangential velocity.
    mixed            : 50% broad and 50% strong_approach attempts.

For targeted supplemental data, continue batch numbers after the broad dataset
(e.g. if broad batches ended at batch 7, start targeted batches at batch 8).
Targeted files are automatically named with a mode suffix, so they can live in
the same encounter_shards folder and be merged with the broad shards.
...
def _sample_masses(rng, mode="broad"):
    """
    Sample masses for the three bodies.

    broad:
        Original log-uniform sampling over the old scalar-NN mass range.
    strong_approach:
        Mostly broad sampling, but with a probe-like component around
        m0 ~ 1, m1 ~ 0.01. This directly increases coverage of the region
        where the v3 diagnostic fixed-state probe exposed local model collapse.
    """
...
def _make_pair_velocity(rng, r_hat, m0, m1, r01, mode="broad"):
    """
    Construct the pair relative velocity.

    broad reproduces the original v3 behaviour: random speed scale and random
    velocity direction.

    strong_approach deliberately samples negative radial velocity and moderate
    tangential velocity in normalised units.  This fills the diagnostic weak
    region without changing the definition of c_opt or the IAS15/leapfrog
    matching methodology.
    """
    v_circ = float(np.sqrt(G * (m0 + m1) / r01))

    if mode == "strong_approach":
        # Normalised radial/tangential ranges chosen from the failed probe
        # region: vr≈-0.8, vt≈0.65.  The ranges are deliberately broad enough
        # to avoid training only on one artificial point.
    ...
    # Original broad v3 velocity sampling.
...
def make_ic(rng, mode="broad"):
    ...
    mode="broad" keeps the original v3 broad random sampling.
    mode="strong_approach" targets large-dt diagnostic weak regions by sampling
    r≈0.08-0.12 AU and strongly approaching velocity states.
    ...
    if mode == "strong_approach":
        # Target the r≈0.10 AU diagnostic region while staying safely inside
        # Zone 3 and away from the 0.05/0.15 boundaries.
        r01 = float(rng.uniform(0.080, 0.120))
    else:
    ...
    # Relative velocity of the close pair. In strong_approach mode this is
    # deliberately biased toward the weak region found by diagnostics.
    ...
    # Distant third body.  Kept deliberately far from Zone 3 so that the
    # training signal remains the pair (0,1) encounter.
...
def build_arg_parser():
    ...
    ap.add_argument("--mode", choices=["broad", "strong_approach", "mixed"], default="broad",
                    help=("Sampling mode. broad reproduces the original v3 generator; "
                          "strong_approach targets the diagnostic weak region; "
                          "mixed uses broad and strong_approach attempts 50/50."))
    ap.add_argument("--no_mode_suffix", action="store_true",
                    help=("Do not append the sampling mode to targeted output filenames. "
                          "Default is to append _strong_approach or _mixed so broad and targeted "
                          "shards can safely live in the same folder."))
...
def main():
    ...
    if args.mode != "broad" and not args.no_mode_suffix:
        file_prefix = f"{file_prefix}_{args.mode}"
    out_file = os.path.join(args.out_dir, f"{file_prefix}_dt{token}_batch{batch_id:03d}.npz")
    summary_file = os.path.join(args.out_dir, f"{file_prefix}_dt{token}_batch{batch_id:03d}_summary.txt")
    ...
    # Deterministic but different seed for every dt/batch/mode.
    mode_offset = {"broad": 0, "strong_approach": 37_000_000, "mixed": 73_000_000}[args.mode]
    seed = int(args.seed) + batch_id * 100000 + int(round(dt * 1_000_000)) + mode_offset
    ...
    if args.mode in ("strong_approach", "mixed"):
        print("  targeted : r≈0.08-0.12 AU, v_rad_norm≈[-1.2,-0.6], v_tan_norm≈[0.4,0.9]")
    ...
    while n_ok < n_target and n_try < max_tries:
        ...
        if v_rad_norm < -0.8:
            n_strong08 += 1
        if (v_rad_norm < -0.6) and (0.4 <= v_tan_norm <= 0.9) and (0.08 <= r01 <= 0.12):
            n_target_window += 1
        n_ok += 1
        ...
        if n_ok == 1 or n_ok == n_target or n_ok % max(1, min(50, n_target // 10)) == 0:
            ...
            print(f"  accepted {n_ok:>5}/{n_target} | tried={n_try:>6} | "
                  ...
                  f"vt_med={np.median(vt_buf):.3f} | "
                  f"vr<-0.6={n_strong06} | {el:.0f}s",
                  flush=True)
    ...
    if n_ok > 0:
        line = (f"dt={dt:.6f}: n={n_ok} | tried={n_try} | pass={n_ok/max(n_try,1):.1%} | "
                ...
                f"v_tan_norm_med={float(np.median(vt_buf)):.5f} | "
                f"vr<-0.6={n_strong06} | vr<-0.8={n_strong08} | "
                f"target_window={n_target_window} | {elapsed:.0f}s")
    ...
    with open(summary_file, "w", encoding="utf-8") as fh:
        ...
        fh.write(f"Strong approach samples v_rad_norm < -0.6: {n_strong06}\n")
        fh.write(f"Strong approach samples v_rad_norm < -0.8: {n_strong08}\n")
        fh.write(f"Target-window samples (r 0.08-0.12, vr<-0.6, vt 0.4-0.9): {n_target_window}\n\n")
    ...
    print(f"Strong approach samples v_rad_norm < -0.6: {n_strong06}")
    print(f"Strong approach samples v_rad_norm < -0.8: {n_strong08}")
    print(f"Target-window samples (r 0.08-0.12, vr<-0.6, vt 0.4-0.9): {n_target_window}")
