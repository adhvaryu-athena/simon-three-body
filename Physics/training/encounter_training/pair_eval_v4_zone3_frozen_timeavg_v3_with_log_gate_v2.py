# pair_eval_v4_zone3_frozen_timeavg_v3_with_log_gate_v2.py
#
# V2 encounter-gate variant:
#   - starts from pair_eval_v4_zone3_frozen_timeavg_v3_with_log_gate.py
#   - keeps the backward/latching gate: if a pair actually touches Zone 1/2,
#     suppress Zone 3 NN for that pair until it exits Zone 3
#   - removes the forward linear prediction gate because it can over-suppress
#     useful Zone 3 corrections at coarse dt, especially dt=0.1
#
...
def simulate_leapfrog_v4_zone3(
    ...
    def make_frozen_zone3_state_geom(rij, r2, r, vel, step_dt):
        ...
        If use_encounter_gate is True, applies the V2 deployment-time gate:
          backward/latch only: skip NN for pairs whose pair_touched_z2 flag
          is True because that pair has actually touched Zone 1/2 during
          the current encounter. The flag resets only when the pair exits
          Zone 3 (r >= nn_thresh).

        V2 deliberately removes the forward linear prediction gate from the
        previous gate file. Suppressed pairs get c=1, matching the no-NN
        baseline in Zone 3.
        """
        ...
        if use_zone3_nn:
            ...
            if use_encounter_gate:
                # V2 backward/latch gate only:
                # If this pair has ACTUALLY touched Zone 1/2 earlier in the
                # current encounter, suppress its Zone 3 NN until it exits
                # Zone 3. Do not use a forward linear prediction gate here;
                # at coarse dt it can falsely suppress useful Zone 3 NN calls.
                gate_suppressed = z0 & pair_touched_z2
...
def main():
    ...
    parser.add_argument("--out_dir", default="v4_zone3_frozen_eval_gate_v2_out")
    ...
    parser.add_argument(
        ...
        help="Disable the deployment-time Zone-3 encounter gate. Default is "
             "enabled. In this V2 file, the gate suppresses NN application "
             "only after a pair has actually touched Zone 1/2, and keeps it "
             "suppressed until that pair exits Zone 3. The forward linear "
             "prediction gate from the previous gate file is removed. Use "
             "this flag for A/B comparison against un-gated NN behaviour.")
