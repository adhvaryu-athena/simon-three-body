...
def simulate_leapfrog_v4_zone3(
    ...
    def make_frozen_zone3_state_geom(rij, r2, r, vel, step_dt):
        ...
        if handoff_guard_enabled:
            handoff_locked[r >= unlock_r] = False
            handoff_locked[r < cfg.adapt_thresh] = True
...
def main():
    ...
    parser.add_argument("--out_dir", default="v4_zone3_handoff_guard_eval_out")
    ...
    parser.add_argument("--skip_handoff_guard", action="store_true", help="Skip the pair-level Zone2 handoff-guard sweep.")
    parser.add_argument("--handoff_unlock_r", type=float, default=0.08, help="Unlock a pair from the Zone2 handoff guard once r >= this AU value.")
    parser.add_argument(
    ...
    guard_rep = None
    if not args.skip_handoff_guard:
        print(f"[eval-v4] Representative v4 Zone 3 HANDOFF-GUARD run dt={dr}...")
        _, p_guard, _, perf_guard = simulate_leapfrog_v4_zone3(
            x0, v0, m, model, cfg, dr, T, ns,
            use_zone3_nn=True, label="v4_zone3_nn_handoff_guard",
            handoff_guard=True, handoff_unlock_r=args.handoff_unlock_r,
        )
        delta_guard = rms_sep(p_guard, pr)
        guard_rep = {
            "perf": perf_guard,
            "final_err": float(delta_guard[-1]),
            "time_avg_rms": float(np.sqrt(np.mean(delta_guard**2))),
        }
        print(f"[eval-v4] guard dt={dr}: final_err={guard_rep['final_err']:.3e} "
              f"time_avg_rms={guard_rep['time_avg_rms']:.3e} "
              f"handoff_skip_frac={perf_guard['zone3_handoff_skip_frac']:.4f}")
    ...
    for dt_val in dts:
            ...
            with open(csv_path, "w", encoding="utf-8") as fcsv:
                if len(nn_log_rows) > 0:
                    ...
                    headers = [
                        ...
                        "gate_fallback", "strong_approach", "handoff_guard_skip", "handoff_locked", "n_zone3_pairs_this_step",
                    ]
        ...
        # ---- handoff-guard NN run at the same dt ----
        if not args.skip_handoff_guard:
            _, pd_g, _, pf_g = simulate_leapfrog_v4_zone3(
                x0, v0, m, model, cfg, dt_val, T, ns,
                use_zone3_nn=True, label="v4_zone3_nn_handoff_guard",
                handoff_guard=True, handoff_unlock_r=args.handoff_unlock_r,
            )
            rms_t_g = rms_sep(pd_g, pr)
            fe_g = float(rms_t_g[-1])
            tar_g = float(np.sqrt(np.mean(rms_t_g**2)))
            tp_g = ns / max(pf_g["total_time_sec"], 1e-12)
            s2_g = perf_r["total_time_sec"] / max(pf_g["total_time_sec"], 1e-12)
            denom_no = max(abs(row.get("time_avg_rms_no_nn", np.nan)), 1e-30)
            denom_ng = max(abs(tar), 1e-30)
            row.update({
                "final_err_guard": fe_g,
                "time_avg_rms_guard": tar_g,
                "throughput_guard": tp_g,
                "time_per_step_sec_guard": pf_g["time_per_step_sec"],
                "total_time_sec_guard": pf_g["total_time_sec"],
                "speedup_guard": s2_g,
                "zone3_nn_frac_guard": pf_g["zone3_nn_frac"],
                "zone3_handoff_skip_frac_guard": pf_g["zone3_handoff_skip_frac"],
                "zone3_gate_fallback_frac_guard": pf_g["zone3_gate_fallback_frac"],
                "c_pred_min_guard": pf_g["c_pred_min"],
                "c_pred_mean_guard": pf_g["c_pred_mean"],
                "c_pred_max_guard": pf_g["c_pred_max"],
                "delta_pct_guard_vs_no_nn": 100.0 * (tar_g - row.get("time_avg_rms_no_nn", np.nan)) / denom_no if np.isfinite(row.get("time_avg_rms_no_nn", np.nan)) else float("nan"),
                "delta_pct_guard_vs_unguarded": 100.0 * (tar_g - tar) / denom_ng,
            })
        else:
            row.update({
                "final_err_guard": float("nan"),
                "time_avg_rms_guard": float("nan"),
                "throughput_guard": float("nan"),
                "time_per_step_sec_guard": float("nan"),
                "total_time_sec_guard": float("nan"),
                "speedup_guard": float("nan"),
                "zone3_nn_frac_guard": float("nan"),
                "zone3_handoff_skip_frac_guard": float("nan"),
                "zone3_gate_fallback_frac_guard": float("nan"),
                "c_pred_min_guard": float("nan"),
                "c_pred_mean_guard": float("nan"),
                "c_pred_max_guard": float("nan"),
                "delta_pct_guard_vs_no_nn": float("nan"),
                "delta_pct_guard_vs_unguarded": float("nan"),
            })
        ...
        if not args.skip_handoff_guard:
            verdict_g = "guard helps vs unguarded" if row["delta_pct_guard_vs_unguarded"] < -0.5 else (
                        "guard hurts vs unguarded" if row["delta_pct_guard_vs_unguarded"] > +0.5 else "guard tie vs unguarded")
            print(f"  dt={dt_val:>6} guard final_err={row['final_err_guard']:.3e} "
                  f"time_avg_rms={row['time_avg_rms_guard']:.3e} "
                  f"total={row['total_time_sec_guard']:.3f}s speedup={row['speedup_guard']:.2f}x "
                  f"zone3NN={row['zone3_nn_frac_guard']:.4f} handoffSkip={row['zone3_handoff_skip_frac_guard']:.4f} "
                  f"| Δ_guard_vs_noNN={row['delta_pct_guard_vs_no_nn']:+.2f}% "
                  f"| Δ_guard_vs_unguarded={row['delta_pct_guard_vs_unguarded']:+.2f}% [{verdict_g}]")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write("method: Revised Zone 2/Zone 3 frozen-c with optional pair-level Zone2 handoff guard\n")
        f.write(f"handoff_guard_unlock_r: {args.handoff_unlock_r:.6f} AU\n")
        ...
        if guard_rep is not None:
            f.write(f"Representative v4 Zone 3 handoff-guard run (dt_rep={dr}):\n")
            f.write(f"  final_err: {guard_rep['final_err']:.6e}\n")
            f.write(f"  time_avg_rms: {guard_rep['time_avg_rms']:.6e}\n")
            f.write(f"  handoff_skip_frac: {guard_rep['perf']['zone3_handoff_skip_frac']:.6f}\n")
            f.write(f"  zone3_nn_frac: {guard_rep['perf']['zone3_nn_frac']:.6f}\n\n")
        ...
        if not args.skip_handoff_guard:
            f.write("\nFrontier sweep (v4 Zone 3 NN with pair-level Zone2 handoff guard):\n")
            f.write("dt\tfinal_err\ttime_avg_rms\tthroughput(samp/s)\ttime_per_step(s)\ttotal_time(s)\tspeedup\tzone3_nn_frac\thandoff_skip_frac\tgate_frac\tc_min\tc_mean\tc_max\n")
            for p in frontier:
                f.write(f"{p['dt']}\t{p['final_err_guard']:.6e}\t{p['time_avg_rms_guard']:.6e}\t"
                        f"{p['throughput_guard']:.3f}\t{p['time_per_step_sec_guard']:.6e}\t"
                        f"{p['total_time_sec_guard']:.6f}\t{p['speedup_guard']:.2f}x\t"
                        f"{p['zone3_nn_frac_guard']:.6f}\t{p['zone3_handoff_skip_frac_guard']:.6f}\t"
                        f"{p['zone3_gate_fallback_frac_guard']:.6f}\t{p['c_pred_min_guard']:.6f}\t"
                        f"{p['c_pred_mean_guard']:.6f}\t{p['c_pred_max_guard']:.6f}\n")

            f.write("\nHandoff-guard comparison (time_avg_rms; negative delta means improvement):\n")
            f.write("dt\tnoNN\tunguarded_NN\tguarded_NN\tguard_vs_noNN_%\tguard_vs_unguarded_%\tverdict\n")
            g_help = g_hurt = g_tie = 0
            for p in frontier:
                dgu = p['delta_pct_guard_vs_unguarded']
                if dgu < -0.5:
                    verdict = "guard_improves_vs_unguarded"
                    g_help += 1
                elif dgu > 0.5:
                    verdict = "guard_worse_vs_unguarded"
                    g_hurt += 1
                else:
                    verdict = "guard_tie_vs_unguarded"
                    g_tie += 1
                f.write(f"{p['dt']}\t{p['time_avg_rms_no_nn']:.6e}\t{p['time_avg_rms']:.6e}\t"
                        f"{p['time_avg_rms_guard']:.6e}\t{p['delta_pct_guard_vs_no_nn']:+.2f}%\t"
                        f"{dgu:+.2f}%\t{verdict}\n")
            f.write(f"\nGuard summary across {len(frontier)} dt values: improves_vs_unguarded={g_help}, "
                    f"worse_vs_unguarded={g_hurt}, tie={g_tie} (threshold: |delta|>0.5%)\n")
        else:
            f.write("\nHandoff-guard sweep was skipped (--skip_handoff_guard).\n")
        ...
        f.write("- handoff-guard mode locks any pair that enters Zone 2 and skips Zone 3 NN for that pair\n")
        f.write("  until r >= handoff_unlock_r; skipped Zone 3 pairs use c=1*F_soft, not Newtonian.\n")
