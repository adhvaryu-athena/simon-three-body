"""
real_system_sun_earth_moon.py

Real-system validation for SIMON using Sun-Earth-Moon initial conditions from
JPL Horizons.

This revised version runs the real-system timestep-frontier experiment:
    ias15 reference
    SIMON adaptive ON
    SIMON adaptive OFF
across multiple dt values.

Main outputs:
    real_system_validation/sun_earth_moon_dt_sweep_T1/
        summary_dt_sweep.txt
        dt_sweep_results.csv
        dt_sweep_results.npz
        timeavg_rms_vs_dt.png
        max_rms_vs_dt.png
        runtime_vs_dt.png
        speed_ratio_vs_dt.png
        earth_moon_range_vs_dt.png
        selected_dt001_distance_comparison.png
        selected_dt001_xy_trajectories.png

Run:
    python real_system_sun_earth_moon.py
...
def summarize_rollout(pos_model, pos_ref, perf, label, dt, bounded_threshold=10.0):
    """
    ...
    finite = bool(np.all(np.isfinite(pos_model)))
...
def write_summary(
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("Real Sun-Earth-Moon timestep frontier using JPL Horizons initial conditions\n")
        f.write("=" * 82 + "\n\n")
        ...
        f.write(
            ...
            f"{'substeps':>10} {'RMS0T(AU)':>13} {'final(AU)':>12} "
            f"{'max(AU)':>12} {'EM_min':>10} {'EM_max':>10} {'bounded':>8}\n"
        )
        f.write("-" * 125 + "\n")
        for mode, rows in [("adaptive_ON", rows_on), ("adaptive_OFF", rows_off)]:
            for r in rows:
...
def main():
    ...
    # Main real-system timestep frontier settings.
    # Keep T=1 first so the experiment remains fast and directly comparable to
    # your corrected base case. Longer T can be added after this sweep is reviewed.
    T = 1.0
    N_SAMPLES = 1000
    DT_VALUES = [0.001, 0.002, 0.004, 0.008, 0.010, 0.012, 0.014, 0.016]

    BASE_OUT_DIR = "real_system_validation"
    CASE_NAME = "sun_earth_moon_dt_sweep_T1_refined"
    OUT_DIR = os.path.join(BASE_OUT_DIR, CASE_NAME)
    os.makedirs(OUT_DIR, exist_ok=True)

    print("\n[real_system] Loading JPL Horizons Sun-Earth-Moon initial conditions.")
    ...
    print("\n[real_system] Loading SIMON model.")
    ...
    print("\n[real_system] Running ias15 reference once.")
    ...
    for dt in DT_VALUES:
        ...
        row_on = summarize_rollout(pos_on, pos_ref, perf_on, "adaptive_ON", dt)
        ...
        row_off = summarize_rollout(pos_off, pos_ref, perf_off, "adaptive_OFF", dt)
    ...
    summary_path = os.path.join(OUT_DIR, "summary_dt_sweep.txt")
    csv_path = os.path.join(OUT_DIR, "dt_sweep_results.csv")
    npz_path = os.path.join(OUT_DIR, "dt_sweep_results.npz")
    ...
    # Also save detailed plots for the dt=0.001 baseline row.
    selected_idx = 0
    plot_pair_distances_three(
        ...
        os.path.join(OUT_DIR, "selected_dt001_distance_comparison.png"),
    )
    plot_xy_trajectories_three(
        pos_ref,
        pos_on_all[selected_idx],
        pos_off_all[selected_idx],
        os.path.join(OUT_DIR, "selected_dt001_xy_trajectories.png"),
    )

    print("\n[real_system] Saved outputs:")
    print(f"  {summary_path}")
    print(f"  {csv_path}")
    print(f"  {npz_path}")
    print(f"  plots in {OUT_DIR}")

    print("\n[real_system] Complete. Paste the printed timestep table back into ChatGPT for interpretation.")
