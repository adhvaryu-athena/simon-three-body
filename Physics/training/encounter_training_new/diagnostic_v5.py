...
class PairCorrectionNN(nn.Module):
    """
    v5 bounded 6-input correction model with residual skip connection.

    Main path:  6 -> hidden -> hidden -> hidden -> 1
    Skip path:  6 -> 1  (linear residual, direct input-to-output)
    raw = main_output + skip_output
    ...
    def __init__(self, hidden: int = 128, c_min: float = 0.25, c_max: float = 3.0):
        ...
        if not (0.0 < float(c_min) < float(c_max)):
            raise ValueError(f"Invalid v5 bounds: c_min={c_min}, c_max={c_max}")
...
def load_model(model_path: str) -> PairCorrectionNN:
    ...
    print("[model] type=v5 bounded output with residual skip connection")
...
def main() -> None:
    ...
    ap.add_argument("--model", default="pair_correction_nn_v5.pt")
    ap.add_argument("--split", default="pair_correction_nn_v5_split.npz")
