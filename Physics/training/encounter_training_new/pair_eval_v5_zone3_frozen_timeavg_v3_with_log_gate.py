...
def simulate_leapfrog_v4_zone3(
    ...
    c_lo = np.inf
    c_hi = -np.inf
    zone3_suppressed_n = np.int64(0)  # Z3-eligible pair-evals suppressed by encounter gate
    ...
    # Encounter gate state: per-pair flag, latches True when pair enters
    # Zone 1 or Zone 2, resets to False when pair returns to Zone 4.
    # Used only when use_encounter_gate AND use_zone3_nn are both True.
    pair_touched_z2 = np.zeros(P, dtype=bool)
    ...
    def make_frozen_zone3_state_geom(rij, r2, r, vel, step_dt):
        ...
        If use_encounter_gate is True, also applies the deployment-time gate:
          backward: skip NN for pairs whose pair_touched_z2 flag is True
                    (still in middle of a Zone-2-touching encounter).
          forward:  skip NN for pairs whose r_next = r + v_rad * step_dt
                    is predicted to enter Zone 2.
        Suppressed pairs get c=1 (same physics as no-NN baseline in Zone 3).
        """
        nonlocal frozen_steps_with_z3_n, c_sum, c_count, c_lo, c_hi
        nonlocal zone3_suppressed_n
        ...
        # ---- Encounter-gate state update (only when gate + NN both active) ----
        # Done unconditionally on the current r at start of this (sub)step.
        # Z1/Z2 latches the flag; Z4 resets it; Z3 leaves it unchanged.
        if use_zone3_nn and use_encounter_gate:
            in_z12 = r < cfg.adapt_thresh
            in_z4 = r >= cfg.nn_thresh
            pair_touched_z2[in_z12] = True
            pair_touched_z2[in_z4] = False
        ...
        if use_zone3_nn:
            # ---- Determine which Z3 pairs the encounter gate suppresses ----
            # Default: all Z3 pairs are "active" (gate disabled or pass both checks).
            # gate_suppressed[p] is True only for pairs we explicitly suppress.
            gate_suppressed = np.zeros(P, dtype=bool)
            if use_encounter_gate:
                # Backward gate: pair_touched_z2 already True for some Z3 pairs.
                back_supp = z0 & pair_touched_z2
                # Forward gate: predict r_next for Z3 pairs and check < adapt_thresh.
                # v_rad in physical units = (v_j - v_i) . r_hat. dr/dt = v_rad.
                rij_z = rij[z0]
                r_z = r[z0]
                r_hat_z = rij_z / (r_z[:, None] + 1e-30)
                vij_z = (vel[jj] - vel[ii])[z0]
                v_rad_phys = np.einsum("ij,ij->i", vij_z, r_hat_z)
                r_next_z = r_z + v_rad_phys * float(step_dt)
                forward_supp_z = r_next_z < cfg.adapt_thresh
                # Lift back to full P-length boolean
                fwd_supp = np.zeros(P, dtype=bool)
                z0_idx = np.where(z0)[0]
                fwd_supp[z0_idx[forward_supp_z]] = True
                gate_suppressed = back_supp | fwd_supp

            # Pairs the NN will actually run on
            active = z0 & ~gate_suppressed
            n_active = int(np.sum(active))
            n_supp = int(np.sum(gate_suppressed & z0))

            # Build NN features only for active pairs and run NN
            if n_active > 0:
                nn_in, v_rad_norm_active = _zone3_features_from_geom(
                    rij, r2, r, vel, step_dt, active)
                c = v4_forward_numpy(nn_in, w)
                r_soft_active = np.sqrt(r2[active] + eps2)
                fb_active = ((r_soft_active < cfg.r_soft_min) | (c < cfg.c_min) |
                             (c > cfg.c_max) | ~np.isfinite(c))
                frozen_c[active] = c
                eligible[active] = True
                fallback[active] = fb_active
                strong[active] = v_rad_norm_active < -0.6
            else:
                c = np.array([], dtype=np.float32)
                v_rad_norm_active = np.array([], dtype=np.float32)
                fb_active = np.array([], dtype=bool)

            # Suppressed pairs: eligible=True, frozen_c=1.0, fallback=False.
            # Numerically c=1*F_soft is the no-NN baseline behaviour in Zone 3.
            if n_supp > 0:
                supp_mask = gate_suppressed & z0
                eligible[supp_mask] = True
                # frozen_c[supp_mask] stays at default 1.0
                # fallback[supp_mask] stays at default False
                strong[supp_mask] = False
                zone3_suppressed_n += n_supp

            # Stats: only increment frozen_steps_with_z3 if NN actually fired
            # at least once on this step (i.e. n_active > 0). If every Z3 pair
            # was suppressed, no NN call happened.
            if n_active > 0:
            ...
            # Records one row per Zone-3 pair on this step (whether NN ran or
            # was suppressed by the encounter gate). For active pairs we log
            # the NN prediction; for suppressed pairs we log c_pred=1.0 and
            # set encounter_gate_suppressed=True so the two are distinguishable.
            if nn_log_rows is not None:
                z3_pair_idx = np.where(z0)[0]
                if len(z3_pair_idx) > 0:
                    # Build full Z3 feature set for logging (covers active + suppressed)
                    r_z3 = r[z0]
                    rij_z3 = rij[z0]
                    r_hat_z3 = rij_z3 / (r_z3[:, None] + 1e-30)
                    vij_z3 = (vel[jj] - vel[ii])[z0]
                    v_rad_full = np.einsum("ij,ij->i", vij_z3, r_hat_z3)
                    v_scale_z3 = np.sqrt(cfg.G * (mi_arr[z0] + mj_arr[z0]) /
                                         (r_z3 + 1e-30))
                    v_rad_norm_z3 = v_rad_full / (v_scale_z3 + 1e-30)
                    v_tan_vec = vij_z3 - v_rad_full[:, None] * r_hat_z3
                    v_tan_mag = np.sqrt(np.einsum("ij,ij->i", v_tan_vec, v_tan_vec)
                                        + 1e-30)
                    v_tan_norm_z3 = v_tan_mag / (v_scale_z3 + 1e-30)
                    r_soft_z3 = np.sqrt(r2[z0] + eps2)
                    c_anal_ref = (r_soft_z3 / (r_z3 + 1e-30)) ** 3
                    n_z3_this_step = int(len(z3_pair_idx))

                    # Compute per-Z3-pair suppressed flag and NN c value to log
                    active_z3 = active[z0]  # bool per Z3 pair (length n_z3)
                    supp_z3 = (~active_z3)
                    # Map active-row c values back into per-Z3 positions
                    c_log = np.full(len(z3_pair_idx), 1.0, dtype=np.float64)
                    fb_log = np.zeros(len(z3_pair_idx), dtype=bool)
                    if n_active > 0:
                        # active_z3 is bool of length n_z3; c covers n_active.
                        c_log[active_z3] = np.where(np.isfinite(c), c.astype(np.float64),
                                                    np.nan)
                        fb_log[active_z3] = fb_active
                    ...
                    for k, p_idx in enumerate(z3_pair_idx):
                        nn_log_rows.append({
                            ...
                            "pair_j": int(jj[p_idx]),
                            "r_au": float(r_z3[k]),
                            "v_rad_norm": float(v_rad_norm_z3[k]),
                            "v_tan_norm": float(v_tan_norm_z3[k]),
                            "m_i": float(mi_arr[p_idx]),
                            "m_j": float(mj_arr[p_idx]),
                            "c_pred": float(c_log[k]),
                            "c_anal_softening": float(c_anal_ref[k]),
                            "gate_fallback": bool(fb_log[k]),
                            "strong_approach": bool(v_rad_norm_z3[k] < -0.6),
                            "n_zone3_pairs_this_step": n_z3_this_step,
                            "encounter_gate_suppressed": bool(supp_z3[k]),
                        })
        else:
            eligible[z0] = True
    ...
    def verlet_no_zone3(x_in, v_in, step_dt, a_in=None):
        ...
        Also: r_min >= nn_thresh on entry implies ALL pairs are in Zone 4,
        so we reset the encounter-gate backward flag here (only when gate
        + NN are both active, to keep other paths bit-identical).
        """
        nonlocal pair_evals_n, far_n
        if use_zone3_nn and use_encounter_gate:
            pair_touched_z2[:] = False
        if a_in is not None:
            a0 = a_in
    ...
    perf = {
        ...
        "use_encounter_gate": bool(use_encounter_gate and use_zone3_nn),
        "zone3_suppressed_pairs": int(zone3_suppressed_n),
        "zone3_suppressed_frac": int(zone3_suppressed_n) / pair_evals,
    }
...
def main():
    ...
    parser.add_argument(
        "--disable_zone3_encounter_gate", action="store_true",
        help="Disable the deployment-time Zone-3 encounter gate. Default is "
             "enabled, which suppresses NN application for any pair whose "
             "current encounter touches Zone 2 (backward) or is predicted to "
             "do so next step (forward linear extrapolation). Use this flag "
             "for A/B comparison against the un-gated NN behaviour.")
    parser.add_argument(
        "--ic", default="IC1", choices=["IC1", "IC3", "IC4", "IC_custom"],
        help="Initial condition to use. IC1=default moderate scattering, "
             "IC3=tight inner pair (more Zone 3 encounters), "
             "IC4=hierarchical wide outer body.")
    ...
    # IC lookup — select via --ic argument.
    ...
    IC_DEFS = {
        ...
        "IC_custom": {
            # Zone 3 resonant pair: body 1 on eccentric orbit with periapsis
            # r_peri=0.09 AU (Zone 3, above Zone 2). Period ~0.40 yr.
            # Expected ~100 Zone 3 firings per 100 yr at dt=0.08. No Zone 2
            # crossings so encounter gate will not suppress any firings.
            # Body 1 placed at periapsis (v_rad=0, v=4.11 AU/yr tangential).
            # Body 2 at r=2 AU, sub-circular velocity, weakly perturbing.
    ...
    print(f"[eval-v4] Using {args.ic}: m={m.tolist()}")
    ...
    use_enc_gate = not args.disable_zone3_encounter_gate
    print(f"[eval-v4] Zone-3 encounter gate: "
          f"{'ENABLED' if use_enc_gate else 'DISABLED'}")
    frontier = []
    for dt_val in dts:
        # ---- NN run ----
        ...
        _, pd, _, pf = simulate_leapfrog_v4_zone3(
            ...
            use_encounter_gate=use_enc_gate,
        )
        ...
        print(f"  dt={dt_val:>6} NN    final_err={fe:.3e} time_avg_rms={tar:.3e} "
              ...
              f"gate={pf['zone3_gate_fallback_frac']:.2%} "
              f"encGate_supp={pf['zone3_suppressed_pairs']} "
              f"speedup={s2:.2f}x")
        if do_no_nn_sweep:
