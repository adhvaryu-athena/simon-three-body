...
OUT_DIR            = "population_validation_output_dt08"
os.makedirs(OUT_DIR, exist_ok=True)

# Simulation parameters (identical to main paper)
DT                 = 0.08          # yr -- paper operational timestep
T_SIM              = 100.0         # yr
...
# Constrained to SIMON's validated non-crossing / non-persistent-close-encounter regime.
R1_RANGE = (1.0, 3.0)      # inner body separation (AU)
R2_RANGE = (3.0, 8.0)      # outer body separation (AU)

# Velocity scale factors (as fraction of circular velocity)
# Avoid highly eccentric plunging orbits that create persistent close encounters.
F1_RANGE = (0.6, 1.2)      # inner body: bounded, moderate eccentricity
F2_RANGE = (0.7, 1.1)      # outer body: avoid plunging outer trajectories

# IC rejection thresholds
MIN_INITIAL_SEPARATION = 0.20   # AU; avoid immediate close encounters
MIN_INNER_PERIAPSIS   = 0.20   # AU; keep inner orbit outside persistent substep regime
OUTER_PERIAPSIS_FACTOR = 1.50  # require q_outer > 1.5 * r1 for non-crossing hierarchy
MAX_ATTEMPTS = 100 * N_POP     # prevents infinite loop if filters become too strict
...
def generate_random_ic(rng, G=1.0):
    ...
    # Reject ICs whose estimated periapsis falls outside SIMON's intended
    # population-validation regime.
    q1_est = estimate_periapsis(r1, v1, mu1)
    if q1_est is None or q1_est < MIN_INNER_PERIAPSIS:
        return None

    q2_est = estimate_periapsis(r2, v2, mu2)
    if q2_est is None or q2_est < OUTER_PERIAPSIS_FACTOR * r1:
        return None
    ...
    return x0, v0, m, float(E_total), float(r1), float(r2), float(q1_est), float(q2_est)
...
def main():
    ...
    print(f"  Generated {N_POP} bound, non-crossing ICs from {attempts} attempts "
    ...
    for ic_idx, (x0, v0, m, E0, r1_init, r2_init, q1_est, q2_est) in enumerate(ics):
        ...
        try:
            ...
            # Divergence rate lambda is meaningful only when both integrators
            # keep the configuration bounded.
    ...
    lines = [
        ...
        f"  T=100yr, dt=0.08yr  (speed-oriented population timestep)",
        ...
        f"q_outer >= {OUTER_PERIAPSIS_FACTOR:.1f} r1, "
        ...
        f"distribution) and separations drawn from a non-crossing hierarchical",
        f"population filtered to avoid persistent close encounters below the",
        f"validated SIMON operating regime.",
