...
class HybridConfig:
    ...
    dir_cos_min: float = 0.866  # Vector NN direction gate: cos(30°)
...
def plot_divergence(times, deltas, slopes, windows, path):
    ...
    with plt.rc_context({
        ...
        ax.semilogy(
            ...
            label=f"SIMON (analytic direction)\nλ ≈ {slopes['simon']:.4f}/yr"
        )
        ax.semilogy(
            times, deltas['vector'],
            color=C['vector'], lw=1.8,
            label=f"Vector NN (learned direction)\nλ ≈ {slopes['vector']:.4f}/yr"
        )

        ax.axvspan(
            windows['simon'][0], windows['simon'][1],
        ...
        # Focus the log scale on the physically meaningful plotted range.
        # This hides the near-zero initial point only visually; it does not change
        # any computed RMS error, slope, or summary result.
        finite_positive = np.concatenate([
            np.asarray(deltas['simon'])[np.asarray(deltas['simon']) > 0],
            np.asarray(deltas['vector'])[np.asarray(deltas['vector']) > 0],
        ])
        y_max = max(2e2, float(np.nanmax(finite_positive)) * 1.2)
...
def plot_trajectories(pr, ps, pv, path):
    # Local font override for this paper figure only.
    # This changes only chart formatting, not trajectories or results.
    with plt.rc_context({
        "font.size": 10,
        "axes.titlesize": 12,
        "axes.labelsize": 10,
        ...
        for i in range(N):
            ...
            ax.plot(pr[:, i, 0], pr[:, i, 1], '-',
                    color=C['ias15'], lw=1.4, label='ias15')
            ax.plot(ps[:, i, 0], ps[:, i, 1], '--',
                    color=C['simon'], lw=1.3, label='SIMON')
            ax.plot(pv[:, i, 0], pv[:, i, 1], ':',
                    color=C['vector'], lw=1.3, label='Vector NN')
            ...
            # Show the legend only once to avoid covering trajectories repeatedly.
            if i == 0:
