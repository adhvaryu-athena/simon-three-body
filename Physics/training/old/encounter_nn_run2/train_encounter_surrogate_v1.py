...
Train an encounter-level residual surrogate for SIMON Zone-3 encounter windows.

Scientific purpose
------------------
This is NOT the scalar Zone-3 c-correction model.  It trains a bounded-window
encounter surrogate:

    input  X = X_rel18  (relative 3-body state at Zone-3 encounter start)
    target Y = [residual_x, residual_v] flattened to 18 values
             = [IAS15_final - noNN_final]

At inference on an encounter window:

    corrected_final_state = noNN_final_state + NN_predicted_residual

The first success test is held-out window accuracy:

    error(IAS15, corrected_final) < error(IAS15, noNN_final)
...
merge_encounter_surrogate_shards_v1.py.  Required fields include:
    ...
    r_pair, v_rad_norm, v_tan_norm, relE_ias, relE_nonn,
    pos_residual_rms, vel_residual_rms, category_id

Example
-------
    python -B train_encounter_surrogate_v1.py \
        --data encounter_surrogate_v1_proto.npz \
        --epochs 3000 \
        --hidden 128 \
        --output encounter_surrogate_v1_proto.pt
...
-------
    ...
    <output stem>_scatter.png           noNN vs corrected error scatter if matplotlib is available
"""
...
class EncounterResidualMLP(nn.Module):
    ...
    def __init__(self, hidden: int = 128, dropout: float = 0.0):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(hidden, hidden), nn.SiLU(), nn.Dropout(dropout),
            nn.Linear(hidden, max(64, hidden // 2)), nn.SiLU(),
            nn.Linear(max(64, hidden // 2), 18),
        )
...
def make_stratified_splits(raw: Dict[str, np.ndarray], seed: int,
    ...
    """
    Stratify by category_id so strong/weak/recede are represented in all splits.
    """
    ...
    for c in sorted(set(cat.tolist())):
        ...
        rng.shuffle(idx)
        if len(idx) < 3:
            # Put tiny categories into train, but warn through summary later.
            train_parts.append(idx)
            continue
        n_train = int(round(train_frac * len(idx)))
        n_val = int(round(val_frac * len(idx)))
        # Ensure test is not empty for categories with at least 3 samples.
...
def compute_metrics(raw: Dict[str, np.ndarray], indices: np.ndarray,
                    pred_resid: np.ndarray) -> Dict[str, float]:
    """
    ...
    # Residual target error, not final-state error.
    ...
    # Unsafe correction heuristic: correction makes position error > 2x baseline.
    unsafe = corr_pos > (2.0 * base_pos + 1e-12)
    ...
    return {
        ...
        "unsafe_pos_frac": float(np.mean(unsafe)),
    }
...
def category_table(raw: Dict[str, np.ndarray], indices: np.ndarray,
                   pred_resid: np.ndarray) -> List[str]:
    lines: List[str] = []
    ...
    for c in sorted(set(cat.tolist())):
        ...
        m = compute_metrics(raw, idx_c, pred_c)
        name = CATEGORY_NAMES.get(int(c), f"category_{int(c)}")
        lines.append(
            f"  {name:16s} n={int(m['n']):5d} "
            f"base_pos_med={m['base_pos_median']:.4e} corr_pos_med={m['corr_pos_median']:.4e} "
            f"impr_med={m['pos_improvement_median_pct']:+7.2f}% success={m['pos_success_frac']:.1%} "
            f"unsafe={m['unsafe_pos_frac']:.1%}"
        )
    return lines
...
def train_one_model(args) -> None:
    ...
    # Training in normalized target units.
    ...
    ds = TensorDataset(X_train, Y_train_norm)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb_cpu, yb_cpu in dl:
            ...
            opt.step()
            loss_sum += float(loss.detach().cpu().item()) * len(xb_cpu)
            n_seen += len(xb_cpu)

        train_loss = loss_sum / max(n_seen, 1)
        train_losses.append(train_loss)

        # Validation score in physical units: corrected position median + small velocity term.
        pred_val = predict_residual(model, X[val_idx], device=device, batch_size=args.eval_batch_size)
        vm = compute_metrics(raw, val_idx, pred_val)
        val_score = vm["corr_pos_median"] + args.val_vel_weight * vm["corr_vel_median"]
        ...
        if epoch == 1 or epoch % args.print_every == 0 or epoch == args.epochs:
            print(
                ...
                f"val_score={val_score:.6e} val_corr_pos_med={vm['corr_pos_median']:.6e} "
                f"val_base_pos_med={vm['base_pos_median']:.6e} "
                f"val_impr_med={vm['pos_improvement_median_pct']:+.2f}% "
    ...
    metrics_train = compute_metrics(raw, train_idx, pred_train)
    metrics_val = compute_metrics(raw, val_idx, pred_val)
    metrics_test = compute_metrics(raw, test_idx, pred_test)
    ...
    x_corr = np.asarray(raw["x_nonn_final"], dtype=np.float32)[test_idx] + prx
    v_corr = np.asarray(raw["v_nonn_final"], dtype=np.float32)[test_idx] + prv
    np.savez_compressed(
        ...
        x_ias_final=np.asarray(raw["x_ias_final"], dtype=np.float32)[test_idx],
        v_ias_final=np.asarray(raw["v_ias_final"], dtype=np.float32)[test_idx],
        x_nonn_final=np.asarray(raw["x_nonn_final"], dtype=np.float32)[test_idx],
        v_nonn_final=np.asarray(raw["v_nonn_final"], dtype=np.float32)[test_idx],
    )
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        w("Encounter-level surrogate v1 training summary")
        w("=" * 80)
        w(f"data: {args.data}")
        ...
        def print_metric_block(name: str, m: Dict[str, float]):
            ...
            for k in [
                ...
                "vel_improvement_mean_pct", "vel_improvement_median_pct", "vel_success_frac",
                "resid_pos_err_median", "resid_vel_err_median",
            ]:
                w(f"  {k:32s}: {m[k]:.8g}")
            w("")
        ...
        w("-" * 80)
        for line in category_table(raw, test_idx, pred_test):
            w(line)
        w("")
        ...
        w("Verdict")
        w("-" * 80)
        if metrics_test["pos_success_frac"] >= 0.60 and metrics_test["pos_improvement_median_pct"] > 0.0:
            w("PROCEED: surrogate improves held-out encounter-exit position error on most samples.")
        elif metrics_test["pos_improvement_median_pct"] > 0.0:
            w("PROCEED WITH CAUTION: median position improves, but success fraction is not strong.")
        else:
            w("STOP/REASSESS: surrogate does not improve median held-out position error.")

    # Figures.
    if HAS_MPL:
        fig_path = stem + "_loss.png"
        ...
        plt.title("Encounter surrogate training curve")
        plt.legend()
        plt.tight_layout()
        plt.savefig(fig_path)
        plt.close()

        # Scatter: noNN baseline error vs corrected error on test set.
        idx = test_idx
        ntest = len(idx)
        prx = pred_test[:, :9].reshape(ntest, 3, 3).astype(np.float64)
        x_ias = np.asarray(raw["x_ias_final"], dtype=np.float64)[idx]
        x_no = np.asarray(raw["x_nonn_final"], dtype=np.float64)[idx]
        base_pos = _rms_state_pos(x_no, x_ias)
        corr_pos = _rms_state_pos(x_no + prx, x_ias)
        lo = max(1e-10, min(base_pos.min(), corr_pos.min()) * 0.8)
        hi = max(base_pos.max(), corr_pos.max()) * 1.2
        scatter_path = stem + "_scatter.png"
        plt.figure(figsize=(5.5, 5))
        plt.scatter(base_pos, corr_pos, s=18, alpha=0.7)
        plt.plot([lo, hi], [lo, hi], "k--", lw=1.2, label="no change")
        plt.xscale("log")
        plt.yscale("log")
        plt.xlabel("noNN position error vs IAS15")
        plt.ylabel("corrected position error vs IAS15")
        plt.title("Held-out encounter-window correction")
        plt.legend()
        plt.tight_layout()
...
# =============================================================================
def parse_args():
    ap = argparse.ArgumentParser(description="Train encounter-level residual surrogate v1.")
    ap.add_argument("--data", default="encounter_surrogate_v1_proto.npz",
                    help="Merged encounter surrogate dataset .npz")
    ap.add_argument("--output", default="encounter_surrogate_v1_proto.pt",
    ...
    ap.add_argument("--vel-weight", type=float, default=0.1)
    ap.add_argument("--val-vel-weight", type=float, default=0.1,
                    help="Validation selection score = corr_pos_median + this*corr_vel_median")
