...
# =============================================================================
# Encounter-level residual NN model: NumPy deployment
# =============================================================================
@dataclass
class EncounterNumpyWeights:
    """
    Lightweight NumPy representation of the final encounter residual MLP.

    This mirrors the scalar Zone-3 evaluator pattern: load the PyTorch checkpoint
    once, extract weights to NumPy once, then do pure NumPy inference inside the
    rollout. This avoids tiny one-sample CUDA/PyTorch calls during the simulation.
...
def _linear_layer_indices(sd: Dict[str, torch.Tensor]) -> List[int]:
    """Return sorted numeric indices for net.<idx>.weight linear layers."""
    idxs: List[int] = []
    for k, v in sd.items():
...
def load_encounter_numpy(model_path: str) -> EncounterNumpyWeights:
    """
    Load encounter residual checkpoint and extract a pure NumPy MLP.

    Expected final successful model architecture from the velocity-safe trainer:
        18 -> 128 -> 128 -> 128 -> 64 -> 18
    but this loader deliberately infers the actual Linear layers from the
    checkpoint, so it is robust to the final hidden-width/index pattern.
    """
    ...
    if missing:
        raise KeyError(
            f"Encounter checkpoint missing {missing}. This evaluator expects the "
            "velocity-safe 18D residual model with input/target normalisation buffers."
        )

    idxs = _linear_layer_indices(sd)
    if not idxs:
        raise KeyError("No Linear layers found in checkpoint under keys like net.<idx>.weight.")
    ...
    for idx in idxs:
        ...
        bk = f"net.{idx}.bias"
        if bk not in sd:
            raise KeyError(f"Missing bias for linear layer {wk}: expected {bk}")
    ...
    if shapes[0][0] != 18:
        raise ValueError(f"Encounter model must take 18 inputs; first layer shape is in/out={shapes[0]}")
    if shapes[-1][1] != 18:
        raise ValueError(f"Encounter model must output 18 residuals; final layer shape is in/out={shapes[-1]}")

    input_mean = sd["input_mean"].detach().cpu().numpy().astype(np.float32).reshape(-1).copy()
    input_std = sd["input_std"].detach().cpu().numpy().astype(np.float32).reshape(-1).copy()
    target_mean = sd["target_mean"].detach().cpu().numpy().astype(np.float32).reshape(-1).copy()
    target_std = sd["target_std"].detach().cpu().numpy().astype(np.float32).reshape(-1).copy()

    for name, arr, n in [
        ("input_mean", input_mean, 18),
        ("input_std", input_std, 18),
        ("target_mean", target_mean, 18),
        ("target_std", target_std, 18),
    ]:
        if arr.shape != (n,):
    ...
    # Avoid division pathologies if any std is accidentally tiny.
...
def _silu_np_enc(x: np.ndarray) -> np.ndarray:
    # Stable enough for normalized MLP activations; clip to avoid overflow.
    return x * (1.0 / (1.0 + np.exp(-np.clip(x, -60.0, 60.0))))


def encounter_forward_numpy(X: np.ndarray, w: EncounterNumpyWeights) -> np.ndarray:
    """Pure NumPy forward pass returning one 18D residual in physical units."""
    ...
    for layer_i, (W, b) in enumerate(w.layers):
        ...
        if layer_i != len(w.layers) - 1:
            z = _silu_np_enc(z).astype(np.float32)
    y = z.reshape(18).astype(np.float32)
    y = y * (w.target_std + np.float32(1e-8)) + w.target_mean
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="SIMON encounterNN full ablation evaluator for IC1 dt=0.08 with NumPy encounterNN inference.")
    ...
    parser.add_argument("--out_dir", default="encounterNN_ablation_v3_numpy_out")
    ...
    parser.add_argument("--device", default="numpy", help="Deprecated/ignored in v3; encounterNN inference is pure NumPy.")
    ...
    print("=" * 88)
    print("SIMON ENCOUNTERNN FULL ABLATION v3 NUMPY")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("SIMON encounterNN ablation v3 NumPy\n")
        f.write("=" * 88 + "\n")
        ...
        f.write("The noNN and scalarNN baselines are run through the original v4 evaluator path. The encounterNN path reuses the optimized noNN kernel strategy and performs the rare residual NN inference with pure NumPy weights, avoiding CUDA/PyTorch calls inside the rollout.\n\n")
        ...
        f.write("- Runtime can vary slightly across runs and hardware; encounterNN inference is pure NumPy in this v3 file, so no CUDA launch overhead is included in the timed rollout.\n")
