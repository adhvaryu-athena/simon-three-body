"""
generate_encounter_surrogate_data_v2_event_targeted.py

Encounter-level surrogate data generator for SIMON (v2 event-targeted, dt=0.08).

Purpose
-------
Generate full-state encounter-window training data for a future neural
encounter surrogate, including an event-targeted mode around the real dt=0.08 rollout encounter.  This is NOT the old scalar-c correction dataset.
...
Recommended smoke test:
    python -B generate_encounter_surrogate_data_v2_event_targeted.py --n 10 --batch 1 --dt 0.08 --mode mixed --window-years 0.5

Recommended first real shards:
    python -B generate_encounter_surrogate_data_v2_event_targeted.py --n 50 --batch 2 --dt 0.08 --mode mixed --window-years 0.5
...
def choose_empirical_row(rng: np.random.RandomState, dist: Optional[Dict[str, np.ndarray]], mode: str) -> Tuple[float, float, float, float, float, int]:
    ...
    if dist is None:
        ...
        if mode == "event_strong_approach":
            # Target the real dt=0.08 rollout event that was OOD for v2:
            # vr≈-1.1, vt≈0.85, r≈0.139 AU.  Keep ranges broad enough
            # to avoid training on a single artificial point.
            r = float(rng.uniform(0.100, 0.145))
            log_mi = float(rng.uniform(np.log(0.003), np.log(0.030)))
            log_mj = float(rng.uniform(np.log(0.002), np.log(0.020)))
            vr = float(rng.uniform(-1.40, -0.90))
            vt = float(rng.uniform(0.70, 1.10))
        elif mode == "strong_approach":
    ...
    if mode == "event_strong_approach":
        # Do NOT rely only on empirical rows here.  The original dt=0.08
        # proposal under-covered vr≈-1.1, so this mode is explicitly
        # sampled below after the row-choice step.
        mask = (vr_all < -0.85) & (vt_all >= 0.60) & (vt_all <= 1.20) & (r_all >= 0.095) & (r_all <= 0.148)
    elif mode == "strong_approach":
...
def construct_state(
    ...
    if submode == "event_strong_approach":
        # Explicitly enrich the real-rollout OOD region:
        #   r_pair≈0.139 AU, v_rad_norm≈-1.1, v_tan_norm≈0.85, dt=0.08.
        # The active pair in IC1 is bodies 1-2 with masses ~0.01 and ~0.005;
        # the third body is the primary with mass ~1.0.
        r_pair = float(rng.uniform(0.100, 0.145))
        vr_norm = float(rng.uniform(-1.40, -0.90))
        vt_norm = float(rng.uniform(0.70, 1.10))
        if rng.rand() < 0.70:
            log_mi = float(rng.uniform(np.log(0.006), np.log(0.020)))
            log_mj = float(rng.uniform(np.log(0.003), np.log(0.012)))
        else:
            log_mi = float(rng.uniform(np.log(0.003), np.log(0.030)))
            log_mj = float(rng.uniform(np.log(0.002), np.log(0.020)))
        source_row_idx = -2
    ...
    if submode == "event_strong_approach":
        # IC1-like primary/third body mass around 1.0, with moderate spread.
        log_mk = float(rng.uniform(np.log(0.5), np.log(2.0)))
    ...
    category_id = {"strong_approach": 0, "weak_side": 1, "recede": 2, "broad": 3, "event_strong_approach": 0}.get(submode, 4)
    return dict(
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--mode", choices=["broad", "strong_approach", "event_strong_approach", "weak_side", "recede", "mixed"], default="mixed",
                    help="Sampling mode. event_strong_approach targets the real dt=0.08 rollout OOD region near vr=-1.1, vt=0.85, r=0.139.")
    ...
    ap.add_argument("--target-min-r-lo", "--target_min_r_lo", dest="target_min_r_lo", type=float, default=0.0,
                    help="Optional lower bound on IAS15 min_r over window; 0 disables.")
    ap.add_argument("--target-min-r-hi", "--target_min_r_hi", dest="target_min_r_hi", type=float, default=0.0,
                    help="Optional upper bound on IAS15 min_r over window; 0 disables.")
...
def main() -> None:
    ...
    print("=" * 88)
    print("ENCOUNTER-LEVEL SURROGATE DATA GENERATOR v2 EVENT-TARGETED")
    ...
    if args.target_min_r_lo > 0.0 or args.target_min_r_hi > 0.0:
        print(f"  target min_r    : [{args.target_min_r_lo:.6f}, {args.target_min_r_hi:.6f}] AU")
    ...
    while n_ok < int(args.n) and n_try < max_tries:
        ...
        if ref["min_r"] < float(args.min_ias15_r):
            ...
            continue
        if float(args.target_min_r_lo) > 0.0 and ref["min_r"] < float(args.target_min_r_lo):
            rejections["target_min_r_low"] = rejections.get("target_min_r_low", 0) + 1
            continue
        if float(args.target_min_r_hi) > 0.0 and ref["min_r"] > float(args.target_min_r_hi):
            rejections["target_min_r_high"] = rejections.get("target_min_r_high", 0) + 1
            continue
