...
# ── Energy computation (verbatim from energy_drift_eval.py) ──────────────────
def compute_energy_trajectory(pos_arr, vel_arr, m, G=1.0):
    """
    Compute total mechanical energy E(t) = KE(t) + PE(t) at each timestep.
    Uses TRUE unsoftened Newtonian Hamiltonian -- same formula for ias15 and SIMON.
...
for ic_name, ic in ICS.items():
    ...
    for dt in DT_VALUES:
        ...
        if not r["ejected"] and r["max_drift_pct"] > 0:
            dts_fit.append(dt)
...
for ic_name, ic in ICS.items():
    ...
    if dts_plot:
        ax.loglog(dts_plot, drifts_plot, "o-",
                  ...
                  label=f"{ic['label']}  (NN_frac={ic['nn_frac_ref']:.4f})")
    if markers_ej:
    ...
    if slopes.get(ic_name) and slopes[ic_name] is not None:
        ...
        drift_range= np.exp(np.log(s["drifts"]).mean() +
                             s["slope"] * (np.log(dt_range) -
                                           np.log(np.array(s["dts"])).mean()))
        ax.loglog(dt_range, drift_range, "--",
                  color=ic["color"], lw=1.2, alpha=0.6)
        # Annotate slope
        mid_dt    = np.exp(np.log(dt_range).mean()) * 1.05
        mid_drift = np.exp(np.log(drift_range).mean()) * 1.2
        ax.annotate(f"slope={s['slope']:.2f}",
                    xy=(mid_dt, mid_drift),
                    color=ic["color"], fontsize=9, fontweight="bold")

# Theoretical dt^2 reference line
dt_ref_line   = np.array([0.008, 0.12])
drift_ref_line= 1.5 * (dt_ref_line / 0.04)**2
ax.loglog(dt_ref_line, drift_ref_line, ":", color="gray",
          lw=2.0, alpha=0.7, label="Theoretical dt$^2$ (slope=2.0)")
...
ax.set_ylabel("Max fractional energy drift |dE/E0|  (%)", fontsize=12)
ax.set_title("Energy Drift Scales as dt$^2$ — Confirming Leapfrog Attribution\n"
             "IC3 and IC4: NN_frac=0.000 at all dt values (pure leapfrog drift)",
...
for ic_name, ic in ICS.items():
    ...
    ax.set_title(
        f"Energy Drift Time Series — {ic['label']}\n"
        f"|E0| = {abs(ic['E0_actual']):.4f}  |  "
        f"NN_frac at dt=0.04: {ic['nn_frac_ref']:.4f}",
        fontsize=11, fontweight="bold")
...
# ── Figure 3: Grouped bar chart (all IC x dt) ─────────────────────────────────
...
for k, dt in enumerate(DT_VALUES):
    ...
    for ic_name in ICS:
        r = all_results[ic_name][dt]
        drifts.append(r["max_drift_pct"])
        hatches.append("//" if r["ejected"] else "")
    x_pos = x_base + (k - n_dts/2 + 0.5) * bar_w
    col   = DT_COLORS.get(dt, "gray")
    ...
    for bar, h in zip(bars, hatches):
        if h:
            bar.set_hatch(h); bar.set_edgecolor("#555")
    # Value labels
    for bar, val, ej in zip(bars, drifts,
                             [all_results[n][dt]["ejected"] for n in ICS]):
        if val > 0:
            ax.text(bar.get_x()+bar.get_width()/2,
                    ...
                    fontweight="bold",
                    color="#DC2626" if ej else "#1F2937")

ax.set_yscale("log")
ax.axvline(0.5, color="#ccc", lw=0.8)
ax.axvline(1.5, color="#ccc", lw=0.8)
ax.set_xticks(x_base)
ax.set_xticklabels([ic["label"] for ic in ICS.values()], fontsize=11)
ax.set_ylabel("Max |dE/E0| (%, log scale)")
ax.set_title(
    "Energy Drift vs Timestep — All IC x dt Combinations\n"
    "Hatched bars = ejection occurred  |  "
    f"Operational dt={DT_OPERATIONAL}yr",
    fontsize=11, fontweight="bold")
ax.legend(fontsize=9)
