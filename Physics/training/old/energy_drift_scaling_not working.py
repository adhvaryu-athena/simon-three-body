...
def simulate_simon(x0, v0, m, model, cfg, dt, T, n_samples):
    ...
    adapt_thresh = 0.10; max_substeps = 16
    MIN_VIABLE_SUB_DT = 0.01
    ...
    def predict_periapsis(pos, vel, acc, dt_look):
        """
        Predict minimum pairwise separation during the next step of
        length dt_look using a parabolic trajectory estimate.
        Uses ii, jj from enclosing scope.
        """
        best = np.inf
        rij_all = pos[jj] - pos[ii]   # (P, 3)
        vij_all = vel[jj] - vel[ii]   # (P, 3)
        aij_all = acc[jj] - acc[ii]   # (P, 3)
        for k in range(len(ii)):
            r_vec = rij_all[k]
            v_vec = vij_all[k]
            a_vec = aij_all[k]
            vv = np.dot(v_vec, v_vec)
            t_cpa = 0.0 if vv < 1e-30 else -np.dot(r_vec, v_vec) / vv
            t_cpa = float(np.clip(t_cpa, 0.0, dt_look))
            r_pred = r_vec + v_vec * t_cpa + 0.5 * a_vec * t_cpa ** 2
            best = min(best, float(np.sqrt(np.dot(r_pred, r_pred) + 1e-30)))
        return best
    ...
    for _ in range(n_steps):
        ...
        r_min = min(min_pair_dist(x), predict_periapsis(x, v, a, float(dt)))
        if r_min < adapt_thresh:
            n_sub_spatial  = max(2, int(np.ceil(adapt_thresh / r_min)))
            n_sub_temporal = max(1, int(np.ceil(float(dt) / MIN_VIABLE_SUB_DT)))
            n_sub  = min(max_substeps, max(n_sub_spatial, n_sub_temporal))
    ...
    # Guard: floating-point accumulation can leave t_cur just below T,
    # causing the last sample to go unsaved. Fill with final state.
    while si < n_samples:
        pos_out[si] = x; vel_out[si] = v; si += 1
