...
def simulate_leapfrog_encounterNN(
    ...
    """
    Production-style encounterNN ablation path.

    This uses the residual-correction formula:
        corrected_exit = noNN_exit + EncounterNN(entry_state)

    It keeps only lightweight counters. No timeline diagnostics, no alpha sweep,
    no kNN checks, and no plotting inside the hot path.
    """
    x = x0.astype(np.float64).copy()
    v = v0.astype(np.float64).copy()
    t_cur = 0.0
    ...
    stats = {
        "label": "SIMON_encounterNN",
        "steps": 0,
        "dt": float(dt),
        "T_years": float(T),
        "n_samples": int(n_samples),
        "total_substeps": 0,
        "min_r": min_pair_distance_3(x),
        "max_radius": max_radius_state(x),
        "zone1": 0,
        "zone2": 0,
        "zone3": 0,
        "zone4": 0,
    ...
    t_start = time.perf_counter()
    while t_cur < float(T) - 1e-14:
        step_dt_macro = min(float(dt), float(T) - t_cur)
        X, vr, vt, r_pair = make_X_rel18_pair12(x, v, m, dt, cfg)
        in_zone3 = (cfg.adapt_thresh <= r_pair < cfg.nn_thresh)
        if in_zone3:
            stats["gate_candidates"] += 1
        gate_ok = bool(in_zone3 and (vr < float(vr_thresh)) and (t_cur + float(window_years) <= float(T) + 1e-12))

        if gate_ok:
            stats["encounter_attempts"] += 1
            t_event = float(t_cur)
            x_start = x.copy()
            v_start = v.copy()
            ...
            min_r_window = min_pair_distance_3(x)
            a, _, _ = revised_no_nn_acc_simple(x, m, cfg)
            # Advance the underlying SIMON-noNN window exactly as required by the residual formula.
            while remaining > 1e-14:
                step_dt = min(float(dt), remaining)
                x, v, a, rmin, cc, n_sub = no_nn_step_simple(x, v, m, cfg, step_dt, a_in=a)
                t_cur += step_dt
                remaining -= step_dt
                stats["steps"] += 1
                ...
                stats["max_radius"] = max(stats["max_radius"], max_radius_state(x))
                for key in ["zone1", "zone2", "zone3", "zone4"]:
                    stats[key] += cc[key]
                # Fill sample outputs with the noNN path inside the window.
                # The NN correction is applied only at the window exit.
                fill_outputs()

            x_no_exit = x.copy()
            v_no_exit = v.copy()
            dx, dv, pred_pos_norm, pred_vel_norm = predict_encounter_residual(encounter_model, X, device)
            x_corr = x_no_exit + dx
            v_corr = v_no_exit + dv
            if com_project:
            ...
            if use:
                ...
                v = v_corr
                a, _, _ = revised_no_nn_acc_simple(x, m, cfg)
                stats["encounter_used"] += 1
                if not np.isfinite(stats["first_used_t"]):
                    stats["first_used_t"] = t_event
                stats["last_used_t"] = t_event
                stats["pred_pos_norm_sum"] += pred_pos_norm
                stats["pred_vel_norm_sum"] += pred_vel_norm
                # Correct already-filled sample at exact/after exit if needed.
                # Future samples use corrected branch.
                while si < len(times) and t_cur >= times[si] - 1e-12:
                    pos_out[si] = x
                    vel_out[si] = v
                    si += 1
        ...
        # Standard noNN macro step.
        x, v, a, rmin, cc, n_sub = no_nn_step_simple(x, v, m, cfg, step_dt_macro, a_in=a)
        t_cur += step_dt_macro
        stats["steps"] += 1
        stats["total_substeps"] += n_sub
        stats["min_r"] = min(stats["min_r"], rmin)
        ...
        for key in ["zone1", "zone2", "zone3", "zone4"]:
            stats[key] += cc[key]
        fill_outputs()
        if stats["max_radius"] > max_radius_gate:
            break
    ...
    stats["time_per_step_sec"] = stats["total_time_sec"] / max(int(stats["steps"]), 1)
    used = max(int(stats["encounter_used"]), 1)
    stats["pred_pos_norm_mean"] = stats["pred_pos_norm_sum"] / used
    stats["pred_vel_norm_mean"] = stats["pred_vel_norm_sum"] / used
    pair_evals = max(int(stats["zone1"] + stats["zone2"] + stats["zone3"] + stats["zone4"]), 1)
    stats["zone1_frac"] = stats["zone1"] / pair_evals
    stats["zone2_frac"] = stats["zone2"] / pair_evals
    stats["zone3_frac"] = stats["zone3"] / pair_evals
    stats["zone4_far_frac"] = stats["zone4"] / pair_evals
    return times, pos_out, vel_out, stats
...
def main():
    ...
    parser.add_argument("--out_dir", default="encounterNN_ablation_v1_out")
    ...
    print("=" * 88)
    print("SIMON ENCOUNTERNN FULL ABLATION v1")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("SIMON encounterNN ablation v1\n")
        f.write("=" * 88 + "\n")
        ...
        f.write("The noNN and scalarNN baselines are run through the original v4 evaluator path.\n\n")
