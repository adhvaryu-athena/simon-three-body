...
Compact-input encounter-level residual surrogate for SIMON Zone-3 encounter windows.
...
This v3 trainer keeps the SAME target as v2:

    Y = [IAS15_final - noNN_final] = [residual_x_flat9, residual_v_flat9]

but changes the input to an 8-feature physics-informed compact representation:
...
Example
-------
    python -B train_encounter_surrogate_v3_compact.py \
        --data encounter_surrogate_v2_event_enriched.npz \
        --epochs 3000 \
        --hidden 128 \
        --output encounter_surrogate_v3_compact_event_enriched.pt
...
# Model
# =============================================================================
class CompactEncounterResidualMLP(nn.Module):
    """
    Compact-input MLP for a fixed-pair 3-body encounter surrogate.

    Input:
        X_compact8 normalized by input_mean/input_std.
    Output:
    ...
    def __init__(self, hidden: int = 128, dropout: float = 0.0):
        ...
        self.net = nn.Sequential(
            nn.Linear(8, hidden), nn.SiLU(), nn.Dropout(dropout),
            nn.Linear(hidden, hidden), nn.SiLU(), nn.Dropout(dropout),
            nn.Linear(hidden, hidden), nn.SiLU(), nn.Dropout(dropout),
            nn.Linear(hidden, mid), nn.SiLU(),
            nn.Linear(mid, 18),
        )
        self.register_buffer("input_mean", torch.zeros(8, dtype=torch.float32))
        self.register_buffer("input_std", torch.ones(8, dtype=torch.float32))
...
def build_compact_features(raw: Dict[str, np.ndarray]) -> np.ndarray:
    """Return X_compact8 with the exact feature order in FEATURE_NAMES."""
...
def predict_residual(model: CompactEncounterResidualMLP, X: np.ndarray,
...
def train_one_model(args) -> None:
    ...
    model = CompactEncounterResidualMLP(hidden=args.hidden, dropout=args.dropout).to(device)
    with torch.no_grad():
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for start in range(0, n_train, args.batch_size):
            ...
            elem = loss_fn(yn, yb)
            pos_loss = elem[:, pos_slice].mean()
            vel_loss = elem[:, vel_slice].mean()
    ...
    np.savez_compressed(
        ...
        test_idx=test_idx,
        feature_names=np.array(FEATURE_NAMES, dtype=object),
        X_compact_test=X[test_idx].astype(np.float32),
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        w("Encounter-level surrogate v3 compact-input training summary")
        w("=" * 92)
        w(f"data: {args.data}")
        w(f"output: {args.output}")
        w(f"samples: {n}")
        w(f"train/val/test: {len(train_idx)} / {len(val_idx)} / {len(test_idx)}")
        w(f"model: MLP 8 -> hidden={args.hidden} -> 18")
        ...
        if (metrics_test["pos_improvement_median_pct"] > 0.0 and
            ...
            w("PROCEED: compact surrogate improves held-out position and velocity medians with acceptable unsafe fractions.")
        elif metrics_test["pos_improvement_median_pct"] > 0.0 and metrics_test["vel_improvement_median_pct"] > 0.0:
            w("PROCEED WITH CAUTION: compact surrogate improves medians, but unsafe fractions need gating or further tuning.")
        elif metrics_test["pos_improvement_median_pct"] > 0.0:
            w("PARTIAL: compact surrogate improves position, but velocity remains unresolved.")
        else:
            w("STOP/REASSESS: compact surrogate does not improve held-out encounter-exit position error.")
    ...
    if HAS_MPL:
        ...
        plt.title("v3 compact encounter surrogate training curve")
        plt.legend()
        plt.tight_layout()
...
# =============================================================================
def parse_args():
    ap = argparse.ArgumentParser(description="Train compact-input encounter-level residual surrogate v3.")
    ap.add_argument("--data", default="encounter_surrogate_v2_event_enriched.npz",
                    help="Merged encounter surrogate dataset .npz")
    ap.add_argument("--output", default="encounter_surrogate_v3_compact_event_enriched.pt",
