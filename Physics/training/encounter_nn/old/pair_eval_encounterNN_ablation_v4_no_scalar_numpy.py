# pair_eval_encounterNN_ablation_v4_no_scalar_numpy.py
#
# Focused SIMON encounter-level residual-NN evaluator.
#
# Purpose
# -------
# Compare only:
...
#   python -B pair_eval_encounterNN_ablation_v4_no_scalar_numpy.py ^
#       --encounter_model encounter_surrogate_v3_rollout_local_velocitysafe.pt ^
#       --dt 0.08 --T 100 --n_samples 5000 ^
#       --out_dir encounterNN_ablation_v4_no_scalar_numpy_out
...
def simulate_simon_encounterNN(
    ...
    while t_cur < float(T) - 1e-14 and steps < n_steps + 10:
        ...
        # Encounter gate at macro-step entry. The final validated model is for pair 1-2.
...
def main():
    ...
    ap.add_argument("--out_dir", default="encounterNN_ablation_v4_no_scalar_numpy_out")
    ...
    print("=" * 88)
    print("SIMON ENCOUNTERNN ABLATION v4 NO-SCALAR NUMPY")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("SIMON encounterNN ablation v4 no-scalar NumPy\n")
        f.write("=" * 88 + "\n")
        f.write("Purpose: compare IAS15, SIMON-noNN, and SIMON-encounterNN on IC1.\n")
        f.write("Only the encounter-level residual NN is evaluated here.\n")
        f.write("EncounterNN deployment formula: corrected_exit = noNN_exit + predicted_residual.\n")
        f.write("EncounterNN inference uses NumPy weights only; no torch/CUDA call occurs inside timed rollout.\n\n")
