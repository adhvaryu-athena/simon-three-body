...
def main():
    ap = argparse.ArgumentParser(
        description="Focused SIMON-noNN vs SIMON-encounterNN evaluator with local encounter-window comparisons. No scalar NN required."
    )
    ...
    ap.add_argument("--out_dir", default="encounterNN_ablation_v6_window_compare_out")
    ...
    ap.add_argument("--physical_window_steps", type=int, default=25,
                    help="Half-width in macro steps for the physically motivated encounter window. For dt=0.08, 25 steps gives ±2.0 yr around event entry, about a 4 yr window.")
    ap.add_argument("--post_window_steps", type=int, default=25,
                    help="Post-correction window length in macro steps after corrected exit. Default matches physical_window_steps.")
    ...
    print("=" * 88)
    print("SIMON ENCOUNTERNN ABLATION v6 FAST-GATE + ENCOUNTER-WINDOW COMPARISON")
    ...
    print(f"  training window : {args.window_years:.3f} yr")
    print(f"  local windows   : physical ±{args.physical_window_steps} steps (=±{args.physical_window_steps*args.dt:.3f} yr), post +{args.post_window_steps} steps (=+{args.post_window_steps*args.dt:.3f} yr)")
    ...
    print("[local] Computing encounter-window metrics from sampled trajectories...")
    window_rows, window_intervals = compute_encounter_window_metrics(
        tr, pr, vr_ref, p_no, v_no, p_en, v_en, event_rows,
        dt=float(args.dt), T=float(args.T),
        physical_window_steps=int(args.physical_window_steps),
        post_window_steps=int(args.post_window_steps),
    )

    event_csv = os.path.join(args.out_dir, "encounter_events.csv")
    write_event_csv(event_csv, event_rows)
    window_csv = os.path.join(args.out_dir, "encounter_window_metrics.csv")
    write_window_metrics_csv(window_csv, window_rows)
    local_ts_csv = os.path.join(args.out_dir, "encounter_window_timeseries.csv")
    write_local_timeseries_csv(local_ts_csv, tr, pr, vr_ref, p_no, v_no, p_en, v_en, event_rows, window_intervals)
    local_plot = os.path.join(args.out_dir, "encounter_window_errors.png")
    maybe_write_local_plot(local_plot, tr, pr, vr_ref, p_no, v_no, p_en, v_en, window_intervals)
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("SIMON encounterNN ablation v6 fast-gate no-scalar NumPy with encounter-window comparisons\n")
        f.write("=" * 88 + "\n")
        ...
        f.write("This v6 report adds local encounter-window metrics so the NN effect during the encounter is not hidden by long-horizon chaotic branch divergence.\n\n")
        ...
        f.write(f"physical_window_steps_half_width: {args.physical_window_steps}\n")
        f.write(f"physical_window_half_width_years: {args.physical_window_steps*args.dt:.8f}\n")
        f.write(f"post_window_steps: {args.post_window_steps}\n")
        f.write(f"post_window_years: {args.post_window_steps*args.dt:.8f}\n")
        ...
        f.write("Encounter-window comparison methodology\n")
        f.write("-" * 88 + "\n")
        f.write("Why this exists: the full 0-100 yr metric can be dominated by chaotic branch divergence after the encounter. These local windows ask whether the EncounterNN helped during the encounter itself.\n")
        f.write("Definitions:\n")
        f.write("  event_training_window_entry_to_exit: from gated encounter entry t_start to corrected exit t_exit. This is the model's own 0.5 yr residual window.\n")
        f.write("  event_exit_nearest_sample: nearest stored sample to t_exit; useful as a sampled proxy for the exact exit-state correction.\n")
        f.write("  event_physical_window_entry_centered: [t_start - W*dt, t_start + W*dt], with W = physical_window_steps. This follows the physically motivated equivalent-step window.\n")
        f.write("  event_post_correction_to_physical_end: [t_exit, t_start + W*dt]. This removes the pre-correction part and asks what happens after the correction until the end of the physical encounter window.\n")
        f.write("  event_post_correction_fixed_steps: [t_exit, t_exit + post_window_steps*dt]. This is a fixed post-correction persistence test.\n")
        f.write("Positive gain_pct means EncounterNN is closer to IAS15 than SIMON-noNN. Negative gain_pct means EncounterNN is worse in that window.\n")
        f.write(f"window_metrics_csv: {window_csv}\n")
        f.write(f"window_timeseries_csv: {local_ts_csv}\n")
        if HAS_MPL:
            f.write(f"window_plot_png: {local_plot}\n")
        f.write("\n")

        f.write("Key local-window rows\n")
        f.write("-" * 88 + "\n")
        f.write("scope\tevent_index\tt_start\tt_end\tn_samples\tpos_noNN_timeavg\tpos_encounterNN_timeavg\tpos_gain_pct\tvel_noNN_timeavg\tvel_encounterNN_timeavg\tvel_gain_pct\n")
        key_scopes = {
            "ALL_training_window_entry_to_exit_union",
            "ALL_physical_window_entry_centered_union",
            "ALL_post_correction_to_physical_end_union",
            "ALL_post_correction_fixed_steps_union",
        }
        for row in window_rows:
            if row["scope"] in key_scopes:
                f.write(
                    f"{row['scope']}\t{row['event_index']}\t{float(row['t_start']):.9f}\t{float(row['t_end']):.9f}\t{int(row['n_samples'])}\t"
        ...
        f.write("How to interpret for paper story\n")
        f.write("-" * 88 + "\n")
        f.write("1. If exit/proximal post-correction gains are positive, the EncounterNN learned a meaningful local residual correction.\n")
        f.write("2. If the physical-window gain is weaker than the exit gain, the correction helps locally but is already being diluted by pre-correction samples and early phase drift.\n")
        f.write("3. If full 100-year gain disagrees with local gain, report this as chaotic branch sensitivity rather than as direct failure of the local NN correction.\n")
        f.write("4. The strongest evidence for the NN mechanism is the combination of exit/proximal-window improvement and the event log showing exactly when the model fired.\n\n")
    ...
    print(f"[done] wrote {window_csv}")
    print(f"[done] wrote {local_ts_csv}")
    if HAS_MPL and os.path.exists(local_plot):
        print(f"[done] wrote {local_plot}")
