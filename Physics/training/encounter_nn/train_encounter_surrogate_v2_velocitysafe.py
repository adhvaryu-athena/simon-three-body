...
def train_one_model(args) -> None:
    ...
    ds = TensorDataset(X_train, Y_train_norm, m_train, x_no_train, v_no_train, x_ias_train, v_ias_train)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        loss_sum = 0.0
        n_seen = 0
        for xb_cpu, yb_cpu, mb_cpu, xno_cpu, vno_cpu, xias_cpu, vias_cpu in dl:
            xb = xb_cpu.to(device, non_blocking=True)
            yb = yb_cpu.to(device, non_blocking=True)
            mb = mb_cpu.to(device, non_blocking=True)
            xno = xno_cpu.to(device, non_blocking=True)
            vno = vno_cpu.to(device, non_blocking=True)
            xias = xias_cpu.to(device, non_blocking=True)
            vias = vias_cpu.to(device, non_blocking=True)
