...
Output figures:
    ...
    fig_step3g_strong_approach_coverage.png
"""
...
def main():
    ...
    print("\n" + "─" * 75)
    print("STRONG-APPROACH COVERAGE BY dt")
    print("─" * 75)
    print("  This checks the region targeted after the diagnostic probe failure:")
    print("  v_rad_norm < -0.6 and v_rad_norm < -0.8, especially near r=0.08-0.12 AU and vt=0.4-0.9.")
    print(f"  {'dt':>7} | {'n':>6} | {'vr<-0.6':>8} | {'vr<-0.8':>8} | {'targetwin':>9} | {'c_med(vr<-0.6)':>15} | {'c_med(vr<-0.8)':>15}")
    print("  " + "-" * 91)

    strong_stats = []
    for dt in mdt_vals:
        base = dt_rounded == dt
        strong06 = base & (vr < -0.6)
        strong08 = base & (vr < -0.8)
        targetwin = base & (vr < -0.6) & (r >= 0.08) & (r <= 0.12) & (vt >= 0.4) & (vt <= 0.9)

        c06 = float(np.median(c_opt[strong06])) if np.any(strong06) else float("nan")
        c08 = float(np.median(c_opt[strong08])) if np.any(strong08) else float("nan")
        strong_stats.append({
            "dt": float(dt),
            "n": int(base.sum()),
            "n06": int(strong06.sum()),
            "n08": int(strong08.sum()),
            "ntarget": int(targetwin.sum()),
            "c06": c06,
            "c08": c08,
        })
        print(
            f"  {dt:7.4f} | {int(base.sum()):6d} | {int(strong06.sum()):8d} | "
            f"{int(strong08.sum()):8d} | {int(targetwin.sum()):9d} | {c06:15.4f} | {c08:15.4f}"
        )

    large_dt = [row for row in strong_stats if row["dt"] >= 0.04]
    weak_large = [row for row in large_dt if row["n06"] < 100 or row["ntarget"] < 50]
    if weak_large:
        print("\nSTRONG-APPROACH COVERAGE WARNING:")
        print("  Some large-dt groups still have thin targeted coverage.")
        print("  For targeted supplemental data, aim for at least ~100 samples with vr<-0.6")
        print("  and ~50 target-window samples per large dt before retraining.")
    else:
        print("\nSTRONG-APPROACH COVERAGE: large-dt targeted coverage looks adequate for retraining.")
    ...
    # Figure G: strong-approach coverage by dt.
    fig, ax1 = plt.subplots(figsize=(9, 5))
    if strong_stats:
        dt_s = np.array([row["dt"] for row in strong_stats], dtype=np.float64)
        n06 = np.array([row["n06"] for row in strong_stats], dtype=np.float64)
        n08 = np.array([row["n08"] for row in strong_stats], dtype=np.float64)
        ntarget = np.array([row["ntarget"] for row in strong_stats], dtype=np.float64)
        c06 = np.array([row["c06"] for row in strong_stats], dtype=np.float64)

        width = 0.0025
        ax1.bar(dt_s - width, n06, width=width, alpha=0.55, label="count: vr < -0.6")
        ax1.bar(dt_s, n08, width=width, alpha=0.55, label="count: vr < -0.8")
        ax1.bar(dt_s + width, ntarget, width=width, alpha=0.55, label="count: target window")
        ax1.set_xlabel("macro dt (yr)")
        ax1.set_ylabel("sample count")
        ax1.grid(True, alpha=0.25)

        ax2 = ax1.twinx()
        ax2.plot(dt_s, c06, "ko-", lw=1.5, ms=4, label="median c_opt: vr < -0.6")
        ax2.set_ylabel("median c_opt for vr < -0.6")
        ax2.axhline(1.0, color="k", lw=0.8, ls=":")

        h1, l1 = ax1.get_legend_handles_labels()
        h2, l2 = ax2.get_legend_handles_labels()
        ax1.legend(h1 + h2, l1 + l2, fontsize=8, loc="best")
    ax1.set_title("Zone 3 strong-approach targeted coverage")
    fig.tight_layout()
    fig.savefig(os.path.join(args.out_dir, "fig_step3g_strong_approach_coverage.png"))
    plt.close(fig)
