...
def simulate_leapfrog_hybrid(x0, v0, m, model, cfg, dt, T, n_samples,
    ...
    # Threshold: when r > nn_thresh, correction c is negligibly close to 1.0
    # so we use pure Newtonian and skip the NN entirely.
    # At r = 50*eps = 0.015, c = (r_soft/r)^3 = 1.000000006 — negligible.
    nn_thresh = 50.0 * cfg.eps  # ~0.015
    ...
    for _ in range(n_steps):
        ...
        a, nf = compute_acc(x)
        v = vh + 0.5 * dt_f * a
        t_cur += dt; steps += 1; fb_sum += nf; pair_sum += P
        while si < n_samples and t_cur >= nt - 1e-12:
            pos_out[si] = x; vel_out[si] = v; si += 1
            if si < n_samples: nt = times[si]
...
def plot_overlay_xy_subplots_all_bodies(pr, pm, op):
    N=pr.shape[1]; fig,axes=plt.subplots(1,N,figsize=(5.5*N,4.5))
    if N==1: axes=[axes]
    for i in range(N):
        ax=axes[i]; ax.plot(pr[:,i,0],pr[:,i,1],'-',label="IAS15")
        ax.plot(pm[:,i,0],pm[:,i,1],'--',label="Hybrid")
        ax.set_xlabel("x"); ax.set_ylabel("y"); ax.set_title(f"Body {i} XY (T=100 yr)")
...
def plot_overlay_timeseries_all_bodies(times, pr, pm, op):
    N=pr.shape[1]; fig,axes=plt.subplots(N,2,figsize=(11,3.1*N),sharex=True)
    if N==1: axes=np.array([axes])
    for i in range(N):
        axes[i,0].plot(times,pr[:,i,0],label="IAS15"); axes[i,0].plot(times,pm[:,i,0],'--',label="Hybrid")
        axes[i,0].set_ylabel(f"body {i} x(t)"); axes[i,0].grid(True,alpha=0.3)
        if i==0: axes[i,0].legend(loc="upper left")
        axes[i,1].plot(times,pr[:,i,1],label="IAS15"); axes[i,1].plot(times,pm[:,i,1],'--',label="Hybrid")
        axes[i,1].set_ylabel(f"body {i} y(t)"); axes[i,1].grid(True,alpha=0.3)
        if i==0: axes[i,1].legend()
    axes[-1,0].set_xlabel("time (yr)"); axes[-1,1].set_xlabel("time (yr)")
    fig.suptitle("Trajectory Time-Series Overlay"); plt.tight_layout(); plt.savefig(op,dpi=200); plt.close()
...
def plot_speed_accuracy_frontier(pts, op):
    plt.figure(); xs=[p["throughput_samp_per_sec"] for p in pts]; ys=[p["final_err"] for p in pts]
    plt.scatter(xs,ys)
    for p in pts: plt.annotate(f"dt={p['dt']}",(p["throughput_samp_per_sec"],p["final_err"]))
    ...
    plt.grid(True,which="both",alpha=0.3); plt.tight_layout(); plt.savefig(op,dpi=200); plt.close()
